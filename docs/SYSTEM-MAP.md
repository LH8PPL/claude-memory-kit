...
| **INSPECT** — `cmk view` (Task 255, v0.6.4; D-414) + the Obsidian vault map (254) | the HUMAN's read port onto the system: an ephemeral loopback UI over facts/trust/edges/health/decisions. Structurally read-only, so it adds a **reader edge only** — it cannot write, cannot log, and therefore cannot perturb the loop it displays. **Task 259 made the edge LIVE without making it two-way**: the UI now polls the tier's own artifacts (audit log, fact-dir mtime, scratchpads) and streams a change *notification* to the open tab, so the window updates itself — still observing, still writing nothing | ✅ | a window |
