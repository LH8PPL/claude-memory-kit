...
One command opens a browser tab on everything the kit has captured: a search box over your facts, each fact's full Why/How with its trust and where it came from, a graph where colour is the cluster a fact belongs to, the rim arc is its trust and an arrow points at what replaced it, the health checks, and the decision journal with retracted entries struck through.

Both lists **page**: Previous / Next with *Showing 51–100 of 2,300* between them, over a stable sort. They are real links — copy one and that page comes back tomorrow, which is why it is a pager rather than an endless scroll: a position in your own archive should be citeable. The **decision journal is searchable** through the same box the facts list uses, and opens at your newest decisions. Search narrows, the pager walks the remainder, and a new query returns you to page 1. Each search hit carries a `strong` / `fair` / `weak` match pill — relative to the best match for that query, stated exactly in the tooltip — so a thin result looks thin.

It **keeps up with you**. A tab you leave open re-reads itself within a couple of seconds of anything changing — a `cmk remember`, a trust change, a `cmk forget`, an edit from a second window — without moving your search, your tier filter or your page. The "as of HH:MM" line also says whether the page is still being kept up to date. Two things are deliberately left alone rather than refreshed under you: the **graph**, which redraws its entire layout (so rather than move every node while you are pointing at one, it tells you memory changed and waits for Refresh), and the **search box** — if you are mid-way through typing a query when something changes, your words are not wiped out from under you. Both just tell you, and wait. And when you press Ctrl-C, the page says so — the viewer is ephemeral, the next `cmk view` picks a different port, so instead of retrying forever or sitting there looking current it stops and tells you to restart and reload.

It's **ephemeral** — it binds a free port on your own machine, serves until you press Ctrl-C, and leaves no background service running. It's **read-only, structurally**: there is no write route to attack or misclick, so changing something means copying the `cmk forget` / `cmk trust` command the page shows you and running it in your shell. And it never leaves your machine: it binds `127.0.0.1`, refuses any other address (it serves your memory with no password — so there is no "expose it" mode), and works with no network at all. The same URLs answer JSON, so it doubles as a local read API. It opens **dark**, with a Light / Dark button in the header and no memory of anything but the theme you last picked.
