...
It **keeps up with you**. A tab you leave open re-reads itself within a couple of seconds of anything changing — a `cmk remember`, a trust change, a `cmk forget`, an edit from a second window — without moving your search, your tier filter or your page. The "as of HH:MM" line also says whether the page is still being kept up to date. The graph is the exception: it redraws its entire layout, so rather than move every node while you are pointing at one, it tells you memory changed and waits for Refresh. And when you press Ctrl-C, the page says so — the viewer is ephemeral, the next `cmk view` picks a different port, so instead of retrying forever or sitting there looking current it stops and tells you to restart and reload.
