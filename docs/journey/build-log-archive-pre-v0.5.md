# Building claude-memory-kit — journey log ARCHIVE (chapters 0–9)

**Status**: FROZEN RECORD · **Relocated**: 2026-07-28 (Task 249) · **Source**: [`build-log.md`](build-log.md)

Chapters **§0 through §9** of the journey log, moved here byte-identical when the live file passed 524 KB / 3,081 lines (Task 249). That range is the whole pre-v0.3 narrative: the frustration that started the project, the research phase, the spec-driven build, the four-spec-generator experiment, the design decisions that shaped v0.1.0, the working-with-an-AI chapter, **every per-task implementation retrospective from Task 1 through Task 108** (v0.2.3, 2026-06-08), the Claude-Code skills-audit side quest, and the two v0.1.0-era planning sections (§8 "What's left to build", §9 "Open questions and post-v0.1 candidates").

**Why the cut lands at §10 and not later.** §10 and its lettered continuations (§10b … §10i) are the per-version arcs, and they are **interleaved, not chronological** — §10d is split in two around §10e, and §10f spans v0.4.3 → v0.5 across the release boundary this task otherwise cuts on. Splitting inside that block would break the narrative it exists to carry, so the whole block stays live along with §11 "How to extend this file". Archiving less was the explicitly-permitted option and this is where it applies.

**This is a point-in-time RECORD, and it is FROZEN.** These chapters describe the kit as it was on the day each was written; they are not updated to match today's code (that is the bug the frozen-record rule prevents). `docs/journey/` is already an exempt path for `validate-docs`'s `counts` family and for the `references` family's `D-nnn` sub-check.

**Going forward.** New narrative goes in [`build-log.md`](build-log.md) under §11's template. When a later pass archives another range of chapters, append it below with its own dated boundary note — do not reword a moved chapter.
