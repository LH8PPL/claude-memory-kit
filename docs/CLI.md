...
**Memory auto-recovery (v0.6.3).** Every `cmk install` — first run or upgrade — also **rescues memory an older version stranded**. Two classes, both automatic, both zero-LLM:

- **Orphaned `context/` folders.** Before v0.6.2 the capture hooks resolved the project root as the agent's raw working directory, so running your agent from a subdirectory forked a second, unread `context/` tier there and wrote real facts into it. Install now scans below the project root (depth-bounded, `node_modules`/dot-dirs/build trees pruned) for such folders — identified by **live state**, so a template scaffold is never touched — and copies their fact files into this project's tier **byte-identically: ids and `created_at` dates are preserved exactly**, so recovered memory keeps its real history. Anything already present here is **skipped, never overwritten**, and a fact you previously `cmk forget`-ed is never resurrected. Non-fact leftovers (`MEMORY.md` bullets, session logs, queue entries) are reported and left for you to review, never auto-merged. The old folder is **left exactly as it was** with a copy-paste delete command printed for your shell — the kit never deletes a memory path. Re-running install is a silent no-op; `cmk doctor` **HC-13** keeps naming the folder until you remove it.
- **Fact files with a missing or invalid id.** A legacy fact whose frontmatter lacks a valid citation id is invisible to recall and gets re-scanned on every session start. Because the kit's ids are content-addressed, install recomputes the id from the file's own body and writes it back (every other byte preserved; a replaced id is kept as `legacy_id`). A file too damaged to repair is **quarantined** into `context/memory/archive/quarantine/` with its bytes intact and reported — never deleted, never silently dropped.

The whole pass is **fail-open**: if anything goes wrong it prints one note and the install completes exactly as it otherwise would.
...
- **JSON on the same routes.** Every view answers HTML or JSON: `/facts`, `/facts.json`, `/api/facts`, or `Accept: application/json`. So the viewer doubles as a local read API (`/api/facts?q=&tier=&limit=&offset=`, `/api/fact/:id`, `/api/graph`, `/api/health`, `/api/decisions`).
- **Live refresh, plus manual, plus honest labels.** An open tab re-reads the view you are on within a couple of seconds of anything changing, without moving your search, tier filter or page. Each view still says when its data was read, and that same line says whether the page is still being kept up to date. The **graph** is the exception — it redraws its whole layout, so it tells you memory changed and waits for Refresh rather than moving every node while you point at one. When you press Ctrl-C the page notices and says so (the next `cmk view` picks a different port, so there is nothing for it to reconnect to). The mechanism is a plain read stream at `/events` — a GET like every other route, no new dependency, no flag to set.
...
- **Only recognised REPAIRS run at all.** An explicit allowlist — `cmk install` / `reindex` / `repair` / `register-crons` / `daily-distill`, plus the `npm install -g …` line HC-8 and HC-9 emit. Everything else is print-only, including real `cmk` commands that are not fixes: `cmk purge`, `cmk uninstall`, `cmk forget` and `cmk redact` are named as yours to run. A verb the kit gains later stays print-only until someone adds it deliberately.

**Exit code.** `--repair` is a request, so its exit code says whether the request was met: **1** if a repair failed, or if one could have been applied but nobody was there to consent (no terminal, no `--yes`); **0** if you were asked and said no, and for the print-only ones (nothing was withheld). Plain `cmk doctor` is unchanged — a WARN there still never affects the exit code.
...
- Also regenerates the tier's `INDEX.md` pointer index and its `MAP.md` Obsidian vault map (both derived, byte-stable, never source of truth). See **[docs/OBSIDIAN.md](OBSIDIAN.md)** for browsing memory in Obsidian.
...
- **If the threshold comes out meaningless, the command REFUSES and exits non-zero** — it does not do a no-op pass and then tell you the corpus is finished. Two ways that happens: the threshold lands at or above the near-duplicate ceiling (everything looks like a duplicate, so there is no "related" band left), or at or below the median of random pairs (the corpus is too uniform for similarity to mean anything). In both cases **nothing is written and nothing is marked as considered**, so the run costs you nothing and a later run over a grown corpus decides again.
- **If you ever need to undo a pass**, `cmk reindex --full` (or `cmk repair --index`) clears the record of which facts have been considered, so the next `cmk autolink` re-decides the whole corpus from scratch. Your links themselves live in your fact files and are never touched by a reindex — to remove a link you edit the fact.
- **One invocation finishes the job.** It works through the whole corpus in small batches (250 facts each), printing progress per batch, and stops when nothing remains — you are never asked to re-run the command to make more progress.
- **Interrupting it costs you nothing** (ADR-0020): each fact is persisted before the next is considered, so Ctrl-C keeps everything already finished, and the next run picks up exactly where it stopped — worked out from your own files, not from a progress file that could go stale. Re-running a finished corpus is a no-op.
- **`--max <n>` is the explicit slice** — do this many facts now and stop, and it tells you how many remain. Use it when you want to sample the result before committing to the whole corpus.
- **The search index is synced for you** at the end of a run that wrote links, so the new edges are immediately live in `cmk links`, `cmk expand` and the viewer's graph. No `cmk reindex` afterwards.
