...
- [ ] **★ KCG-BK1 — `cmk doctor` HC-11 (kiro-cli present).** On a fresh install, `cmk doctor` reports **14** checks and HC-11 = PASS naming `kiro-cli`. FAIL path (scripted `backendCliProbe` returning `present:false`) → HC-11 FAIL with the honest "automatic features degraded, file-only still works" message. **PASS:** 14 checks; HC-11 PASS/FAIL on kiro-cli presence. **FAIL:** a wrong check count or a silent SKIP (the D-270 bug).
