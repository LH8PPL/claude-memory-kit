...
- [ ] **★ KG1b — `cmk doctor` clean (agent-aware HC-1).** `cmk doctor` → **0 fail** on a fresh Kiro install — **15 checks now** (HC-1..HC-15; HC-9 version-drift v0.3.4, HC-10 compaction-liveness v0.4.1, **HC-11 backend-CLI-present v0.4.5**, HC-12 deletion-propagation v0.5.4 — PASS when `kiro-cli` is on PATH, the effective backend for a Kiro install; see ★ KG-BK1, §7b).
