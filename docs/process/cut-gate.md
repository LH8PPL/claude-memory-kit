...
| Check | Move | What it verifies | Reachability |
| --- | --- | --- | --- |
| **★ BK1** | Task 200 — HC-11 | `cmk doctor` emits **14** checks (HC-1..HC-14); HC-11 PASSes when the effective backend agent's CLI is on PATH, and FAILs with an honest "automatic features degraded, file-only still works" message (never a silent no-op) when it's absent | CLI, deterministic (via the injectable `backendCliProbe` seam or a hidden-binary sandbox) |
...
- [ ] **★ G1 — install + doctor clean.**
      ...
      `cmk doctor` → **0 fail** on a fresh install — **14 checks now** (HC-1..HC-14; the 2 memsearch checks were removed in Task 120, HC-8 native-bindings added in v0.3.1, HC-9 version-drift added in v0.3.4, **HC-10 compaction-liveness added in v0.4.1 (Task 167)**, **HC-11 backend-LLM-CLI-present added in v0.4.5 (Task 200/D-272/D-277)**). HC-9 = PASS on a just-installed project (the scaffold marker matches the binary). **HC-10 = SKIP** on a fresh install with no cron registered (it's informational — memory self-heals each session via the lazy roll; only flags a *registered but dead* cron). **HC-11 = PASS** when the effective backend agent's CLI (`claude` on a Claude-Code install) is on PATH; **FAIL** with an honest degrade message if it's absent (see ★ BK1).
      Type:
...
- **★ BK1 — `cmk doctor` HC-11 (backend LLM CLI present).** On a fresh Claude-Code install, `cmk doctor` reports **14** checks and HC-11 = PASS (the `claude` CLI is on PATH). To prove the FAIL path deterministically, use the injectable probe in a script (`runDoctor({ projectRoot, userDir, backendCliProbe: () => ({ agent: 'kiro', bin: 'kiro-cli', present: false, reason: 'kiro-cli not found on PATH' }) })`) — HC-11 = FAIL with an honest "automatic features degraded, file-only still works" message and **no `recoveryCommand` that implies data loss**.
  **PASS:** 14 checks, HC-11 PASS on a healthy install; the injected-absent probe yields a FAIL that names the missing CLI + says capture/search/recall still work. **FAIL:** doctor reports the wrong number of checks, or HC-11 silently SKIPs when the CLI is absent (the D-270 bug), or the message frames it as broken/fatal.
...
- [ ] **F-11**
      - `cmk doctor` → HC-1..HC-14 accurate (HC-8 = native bindings / npm-12 readiness, v0.3.1; HC-9 = version-drift, v0.3.4; HC-10 = compaction-liveness, v0.4.1 — SKIP when no cron registered; HC-11 = backend LLM CLI present, v0.4.5 — PASS/FAIL on the effective backend agent's CLI, never silently skipped) + the trailing **Memory health (informational)** line renders
