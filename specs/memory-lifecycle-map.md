...
| 14 | **forget → tombstone** (forget.mjs) | — | — | — | **moveFactToTombstone** (deleted_at, recoverable) — the archive file is named by the DERIVED [[Archive filename]] (`a` → `_a`), so a case-pair of ids cannot collapse to one file on a case-insensitive FS and destroy the first tombstone (Task 281, D-451) | ✓ correct — the contrast that makes edge #4 inconsistent |
