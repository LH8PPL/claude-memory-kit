...
**Anchor extraction — leads only, all ids in a lead.** `parseDecisionIds` indexes the D-numbers appearing in an **entry lead**: a heading line, or the leading bold span of a list item. The log grew three lead shapes over 400 entries and all three are live (`## <date> — D-405 · TYPE — …`, `## <date> — D-375: TYPE — …`, `- **⚙️ DECISION (<date>) — D-150: …**`). Restricting to leads is what keeps the check's teeth: entry BODIES are dense with citations (`_Relates D-285, D-198._`), and indexing those would make every typo self-anchoring. Within a lead, **all** ids count, not just the first — `- **✅ FIX + RESOLUTION of D-212 (…) — D-213: …**` opens with a citation and carries its own id second, so first-only would drop a genuine anchor. The cost is an occasional false anchor when a lead quotes a never-written number (exactly one in the log today). That trade follows the standing conservatism rule: an extra anchor can only MASK a citation; a missing anchor FAILS THE BUILD on correct prose.

**Frozen records are exempt** — reusing the `counts` family's `isFrozenRecord` path-prefix list verbatim (§17.12, the D-249 frozen-record rule). A research note or an old build-log entry citing a since-superseded or never-numbered entry is history, and "fixing" it is the bug the rule exists to prevent. This also settles the DECISION-LOG itself: it is the **anchor authority**, never a citation source policed against its own anchors. Same stated trade-off as `counts`: a typo in a brand-new log or build-log entry goes unchecked.

**Absent log ⇒ the sub-check is skipped**, and the summary says so rather than printing "0 indexed" — a check that did not run must not read like a check that found nothing. Turning one missing FILE into N citation errors is noise under a fixture root; deletion of the real log is caught by the `registry` family's direction 2 (`docs/journey/DECISION-LOG.md` is a backticked manifest entry).

**What the first run caught — 4 dangling citations, 2 distinct never-written ids.** `D-132` (cited by `specs/tasks.md` Task 147 + `docs/RELEASE-PLAN.md`) was reserved when the log stood at D-131 and the entry was never written — backfilled from the dogfooded facts captured that day. `D-168` (cited by `specs/tasks.md` Task 156, this document's §19.4.2 heading, and the build log) named a real entry that shipped **without its number** — the id was backfilled onto it. Both are the reserved-but-never-written class of D-129 and the ADR-0009/0010 backfills.

**Cross-references:** §17.7 (the family this sub-check joins), §17.12 (the frozen-record list it reuses), D-249, D-129 (the backfill precedent). _Implements: Task 247._
