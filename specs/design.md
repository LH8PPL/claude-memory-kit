...
### 24.1.2 The visual contract (Task 260, 2026-08-03 — the visual pass, D-420)
...
- **One token block per theme, and both are DESIGNED.** A warm neutral ladder — `--ground` / `--panel` / `--sunken` / `--line` / `--line-soft` and three inks — redefined in full under `prefers-color-scheme: dark` rather than inverted. The page and the panel must differ by a real step: the shipped v1 was `#fbfaf8` on `#ffffff`, a 2% delta, which left 1px borders doing 100% of the separation.
