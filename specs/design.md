...
3. **Default-deny, not deny-list.** A recovery string the planner does not RECOGNISE is print-only. The executable set is the kit's own verbs (validated against the live command registry, so it cannot drift) plus the single `npm install -g … --allow-scripts=…` shape HC-8 emits. A deny-list would run every recovery a future check invents until someone remembered to add it, and the failure direction of a missed entry would be "we ran it."
