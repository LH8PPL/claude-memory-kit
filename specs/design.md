...
- **Cap composition (§7.1.2) holds by construction:** the pointer variant is not produced by `enforceCap`, so it is measured against `capBytes` and discarded in favour of the (always cap-honoring) full snapshot under a pathological tiny cap. An empty-memory project emits no pointer at all — the kit does not claim memory it has not got.
