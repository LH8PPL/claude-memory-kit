...
- **A budget that hides records owes a way to REACH them — `?offset=` (Task 269, D-426).** Wave 1 shipped `?limit=` and nothing else, so `/api/facts` and `/api/decisions` answered the first ≤200 records and **no URL existed for the 201st**: 91% of a real fact corpus and 92% of the decision journal were unreachable, and `/api/decisions` said so in its own envelope (`{count: 200, total: 2502, truncated: true}`) while offering nothing to do about it. Both routes now take **`?offset=`** (non-negative integer; absent/empty = 0), and the envelope states the position rather than implying it: **`offset`** (the window's start), **`count`** (rows here), **`total`** (rows the query matched), **`reachable`** (how much of `total` paging can actually get to) and **`has_more`** (`offset + count < reachable` — the pager's whole question, answered by the payload instead of guessed from `count === limit`, which is wrong exactly once: on a corpus whose size is a multiple of the page size, where it offers a Next that leads nowhere).
  ...
  - **A ranked page is a SLICE of one ranking, never a re-run at an SQL offset.** Search pages are served by asking the engine for `offset + limit` rows and slicing — pushing the offset into SQL would rank a *different* candidate window per page (the blend re-ranks the window in JS), and a record could then land on two pages or on none. The price is a **reachable depth**: `reachable = min(total, VIEWER_SEARCH_DEPTH)`, where that constant IS `search.mjs`'s own `SEARCH_MAX_LIMIT`, imported rather than copied so the two cannot drift. Past it the route answers an empty 200 with `has_more: false` and the page says "paging reaches the first N — narrow the search for the rest." Browse mode has no such cap (a plain stable `ORDER BY` pages to the last record), so `reachable === total` there. **The search still RUNS on a beyond-depth page** whose rows are known to be empty — it is what turns an FTS grammar error into a 400 rather than letting the unlimited count throw a 500 for the same typo.
  - **An over-cap `limit` clamps; a malformed `offset` 400s.** Not an inconsistency: an over-cap limit has an obvious sane reading ("give me the most you will"), and an offset past the end has one too — an empty page, served as a 200 that still states `total`. What is left (negative, fractional, non-numeric) has no reading at all.
