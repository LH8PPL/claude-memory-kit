...
- **ONE badge rule, and it is a TINT.** Every pill — trust, tier, state, health status, the graph legend, the active tab — is `background: rgba(<hue>, var(--tint))` under `border: 1px solid rgba(<hue>, var(--tint-line))` with the text in the hue; a variant sets `--hue`/`--hue-rgb` and nothing else. Hues carry their channels as `--*-rgb` so the tint is a **literal rgba pair** — deliberately not a colour-mixing function, whose support the survey could not verify and which both reference implementations avoid. One hue per meaning, unchanged from wave 1.
