...
**Ids are NOT case-insensitively unique (Task 281, D-451).** That lowercase `a` makes `A`/`a` the alphabet's one case-pair, so `P-A234567A` and `P-a234567A` are two distinct, valid, non-colliding ids that a **case-insensitive filesystem** (Windows, macOS by default) cannot tell apart as filenames. Live fact files are unaffected — they are named `<type>_<slug>.md`, and `writeFact` compares the id in an existing file's frontmatter before writing, returning `errorCategory:'collision'` rather than overwriting. The **archive** was affected, because it names files by id; see §6.5 for the filename derivation that closes it. Any NEW id-to-filename mapping added anywhere in the kit must go through that shared derivation (`fact-store.mjs`) rather than interpolating an id into a path.
...
**HC-16** (§14) is the backstop for facts written before this boundary existed — across P, L **and U**, since `cmk persona import` writes `fragments/` as raw bytes and can carry a pre-boundary id in from another machine; `cmk install`'s repair pass covers the same three tiers so the prescription converges (D-446).
...
**The input boundary — what `parse` tolerates, and why it is the ONE place that decides (Task 257, D-403).** A fact file is not always written by the kit: a user hand-edits one, an editor re-saves it, a script rewrites it. The parser therefore owns two byte-level tolerances, both learned from a live bug where a perfectly good fact **silently ceased to exist while sitting untouched in its tier** — invisible to reindex, recall, INDEX/MAP, the edge builder and every fact walk, with the only trace a `skipped — no YAML frontmatter` warning nobody was reading:

| Tolerance | Producer | Precedent |
| --- | --- | --- |
| **CRLF** line endings around the `---` fences | a Windows clone with `autocrlf=true` | D-126 (cut-gate9 H1 — clone reindex found 0 facts) |
| a leading **UTF-8 BOM** (U+FEFF / `EF BB BF`) | a Windows editor; Windows PowerShell 5.1 `Set-Content -Encoding utf8` | D-403 (found by Task 248's live test) |

Two invariants make the BOM fix total rather than partial, and both are contract, not implementation detail:

1. **`parse` strips before the match and returns stripped text on EVERY branch** — the frontmatter branch, the no-frontmatter branch, and both parseError branches. A caller can never receive a BOM back and re-persist it.
2. **`format` never emits a leading BOM**, including one a caller put in the body itself. Together with (1), a BOM'd file **heals on its next legitimate rewrite** (trust override, redact, merge, supersede) rather than needing a repair pass.

The strip delegates to `read-json.mjs`'s `stripBom` — the ONE canonical BOM helper — so the kit has a single BOM policy: exactly **one** leading BOM, matching every other kit BOM site (§13.2, the hook-stdin read, the agent-config reads). Two deliberate NON-changes: content sha1s stay over the **RAW** file bytes (adding or removing a BOM IS a file change and must invalidate a change-detection key, not read as "unchanged"), and `canonicalize()` is left alone — it is **already** BOM-insensitive on the Node side (JS `\s` includes U+FEFF per ECMA-262 WhiteSpace, so the collapse + trim stages absorb it), so a BOM'd fact and its clean twin derive the same id and dedupe against each other. That property is pinned by test rather than assumed. **Cross-implementation divergence — FOUND then RESOLVED (2026-07-27, Task 257 / D-404).**

- **Found (the finding, preserved):** the Python twin's `\s` does NOT match U+FEFF (it is category `Cf`, not whitespace, in the Unicode database Python's `re` follows), so `cmk_canonicalize.generate_id` returned a **different id** than the Node twin for BOM-prefixed input — measured `P-95ZT9JAH` (Python) vs `P-S79MJHFN` (Node) for `"<U+FEFF>Hello world"`. A silent cross-implementation id fork, invisible to the parity harness because **no vector carried a BOM**.
- **Resolved:** **Node is the reference implementation** — its BOM-insensitivity is shipped behavior, so Python moved to match it. `python/cmk_canonicalize/__init__.py` now folds U+FEFF as whitespace via a shared `_WS = r"[\s\ufeff]"` class used by the bullet-marker and whitespace-collapse regexes. **Fold, not a leading strip:** Node does not special-case the leading position — it treats U+FEFF as whitespace in *every* `\s` — so a strip-the-first-BOM shim would still have diverged on mid-string (`"foo<U+FEFF>bar"` → `"foo bar"`), trailing, and bullet-marker-adjacent (`"-<U+FEFF>hello"` → `"hello"`) inputs. Folding is the only choice that keeps every other behavior identical. `str.strip()` and the trailing-punctuation regex are deliberately untouched: the collapse stage has already turned every U+FEFF into a plain space before they run, so widening them would be a provable no-op on a byte-exact contract.
- **Pinned:** seven BOM vectors added to [`fixtures/canonicalize-vectors.json`](../fixtures/canonicalize-vectors.json) (38 → 45), including the two that distinguish fold-from-strip. `scripts/parity-check.mjs` reports byte-identical Node ≡ Python across all 45.
- **Residual, still open, NOT changed:** the two `\s` classes still disagree in the *other* direction — Python's `\s` matches **U+001C–U+001F and U+0085 (NEL)**, JavaScript's does not (measured). No vector covers those, so the contract is untested there. Named rather than silently widened: changing it is a contract decision, not an implementation detail.

**The scratchpad readers do not go through this parser** (bullets carry HTML-comment provenance, not YAML), so each strips independently at its own READ boundary — same rule, different reader. There are two, and both are load-bearing:

- `parseObservationsFromScratchpad` (index-rebuild) — a BOM'd first line blinded the `^##` heading match, silently degrading that section's `heading_path` to the bare filename.
- `readTierBlock` (inject-context) — the more serious one. A bare read sent the BOM **verbatim into the injected session snapshot** (a raw U+FEFF in the model's context), and because `truncateTierToBudget` splits sections on `/^##\s/` and `hasRealContent` tests `/^#{1,6}\s/`, a BOM'd `## ` heading was **not recognised as a section boundary**: the section fell into the always-kept preamble and became **un-droppable**, inverting the §20.3 value-ordered eviction — a low-trust section surviving while a high-trust one is evicted to make room for it. Stripping at the read fixes all three downstream readers at once, which is why the strip belongs there and not in three regexes.

**One strip everywhere — the contract.** Every reader tolerates exactly ONE leading BOM, because that is what `parse` tolerates. A reader that strips MORE than the kit does is not being generous, it is lying: `memory-recovery`'s `classifyFactId` briefly did `parse(stripBom(text))`, so a 2-BOM file classified as *valid* and the recovery pass walked past it as healthy while reindex, recall, INDEX/MAP and the edge builder still could not see it — invisible forever **and** never flagged. Classification now uses the same `parse` contract as everything else, so a ≥2-BOM file is `not-derivable` → quarantined with its **original bytes intact** → named in the report. Visible-and-flagged beats invisible-and-silent, and the user keeps every byte either way.
...
**Project-root resolution in the capture bins (Task 246).** Every capture-hook entry bin (`cmk-capture-prompt` / `-capture-turn` / `-observe-edit`, both the CLI and plugin families) resolves its project root through [`resolveHookProjectRoot`](../packages/cli/src/tier-paths.mjs) — **never bare `process.cwd()`**. Precedence: `CLAUDE_PROJECT_DIR` (Claude Code sets it per hook) → `CMK_PROJECT_DIR` (the kit sets it when a parent bin spawns a detached child, so the child inherits the parent's answer) → walk UP to the nearest `context/`-bearing ancestor, stopping at `$HOME` (the shared `discoverRootUpward`, same the MCP server trusts) → `resolve(cwd)` as the last resort. **Why it exists:** the bins used to pass bare `process.cwd()`, so whenever the agent's cwd was a SUBDIRECTORY of the project (an `npm pack` from `packages/cli`, say), the automatic capture forked a fresh, unread `context/` tier THERE — while `cmk doctor` stayed green because every check reads the ROOT tier. The entry bin resolves ONCE and threads the result to its detached workers via `CMK_PROJECT_DIR`, so the whole capture chain lands in one place. Proven by a live subprocess test (`tests/cli-hook-root-resolution.test.js`): the real bin, run with a subdirectory cwd, writes to the root tier and forks no stray. The **detection** side — a `doctor` check that flags a stray tier from any OTHER cause — is deferred to Task 248 (defense-in-depth; the source is fixed here).
...
4. Moves the original to context/memory/archive/tombstones/<archive-name(id)>.md
   (the DERIVED name — see "Archive filename derivation" below; it equals
   <id>.md for any id without a lowercase `a`)
...
**The compliance scrub + hard purge (Task 96, ADR-0022) — the two verbs past the tombstone.** `cmk redact <id> --pattern <secret>` removes a leaked span from EVERY app-layer copy along the kit's actual dual-write graph — the live fact (frontmatter-aware: the `[redacted: reason date]` marker contains `[`+`:`, so title/field replacement goes parse→replace→re-serialize through frontmatter.mjs, never a raw byte-splice that corrupts the YAML), the tombstoned/superseded archive copies, the dual-written scratchpad bullet (span-replaced in place, the line survives; the per-tier pad list derives from `SCRATCHPADS_BY_TIER` so the L-tier pads — incl. `context.local/private.md`, where the sensitivity screen routes secret-adjacent facts — are covered), the committed `DECISIONS.md` journal entry (title + Why dual-write; span-scrubbed within this fact's `<!-- decision:<id> -->` marker span — the journal is append-only against *deletion*, not against redaction, per the retract-marking precedent), and the INDEX + FTS rows (in-band reindex, the forget/D-85 no-follow-up pattern). A **title-borne secret contaminates the FILENAME too** (writeFact derives it as `<type>_<slugifyFact(title)>.md` — the 2026-07-16 live-test finding): when the title was scrubbed on a LIVE fact and the basename is actually contaminated (it contains the pattern's slug form, or equals the writeFact-canonical name of the leaked title — the latter catches a secret the 60-char slug cap truncated mid-token; a non-canonical filename with a truncated secret slug is the documented residual limit, surfaced via `renameSkipped` when no clean target name exists), the file is renamed to the scrubbed title's slug (id-suffixed on collision); archive copies are id-DERIVED-named and never renamed after the fact (resolveFact depends on it — see "Archive filename derivation" below). Audit-log **path echoes** (the original created-entry's `paths.after`) are scrubbed **JSON-aware** (parse each NDJSON line → replace inside string values → re-stringify, so a pattern containing `"`/`\` is caught in its escaped on-disk form and structure can't corrupt; bounded re-apply if a lock-free concurrent append lands mid-rewrite), covering both the literal pattern and its slug form; neither the redact entry (no `before` path) nor the purge entry (no path at all) ever records a secret-slug path — and purge, which has no pattern, scrubs the removed *filenames* from historical entries instead. The fact survives; the audit entry never carries the secret (pattern identified by LENGTH only). Per-fact scope is explicit: occurrences of the same pattern in other facts, other journal entries, and non-bullet scratchpad lines are counted and REPORTED (`remainingElsewhere`), never silently scrubbed. `cmk purge --hard <id> --yes` is the irreversible escalation: live + every archive copy + bullet lines + the journal entry + index rows gone, NO tombstone, audit kept. Both are **CLI-only by contract — never MCP tools** (this section's separate-destructive-path rule; pinned by cli-redact.test.js down to the import level), and on the committed P tier both print the ADR-0022 git advisory (U/L tiers get a "not committed — rotate anyway" note instead): rotate first, then optionally the documented one-time `filter-repo` team operation — **span-level `--replace-text` after a redact (the fact file survives), path-scoped `--invert-paths` on exactly the purged file(s) after a purge, never the whole `context/` tier** (SECURITY.md runbook) — **the kit never rewrites git history** (D-27: force-push breaks the portable-clone niche).

#### Archive filename derivation — why archive names are not raw ids (Task 281, D-451)

Every file under `archive/tombstones/` and `archive/superseded/` is named by a **derivation of** its fact's id, not the id verbatim. The derivation lives in ONE place, `fact-store.mjs`, and every site that writes, reads, purges, or parses an archive filename goes through it.

**The bug it closes.** The id alphabet keeps both `A` and `a` (§3.1), so `P-A234567A` and `P-a234567A` are distinct valid ids that a **case-insensitive filesystem** maps to the same path. Two such tombstones collapsed into one file and the second write destroyed the first — demonstrated on Windows. The severity is not "an archive file is missing": `rootIdCensus` (§13.2) reads the tombstone archive precisely so a stray-recovery pass never copies a deliberately-forgotten fact back into `memory/`. A destroyed tombstone therefore lets a later recovery **undo a `forget`** — verified live, by deleting one tombstone and watching its stray twin resurrect on the next `cmk install`.

**The derivation.** Escape **both** members of the case-pair: `a` → `_a` and `A` → `_b`. `_` is outside the id alphabet, so the two images are distinct from each other and from every plain character's image. That buys two separate properties, and both are needed:

1. **Injective under case folding.** The per-character folded images (`_a`, `_b`, `b`…`z`, digits) are pairwise distinct AND prefix-free — only the two escapes begin with `_`, and neither is a prefix of the other — so the per-character code is uniquely decodable and two distinct ids can never produce one case-folded name.
2. **Disjoint from the legacy namespace.** Any id containing `a` or `A` yields a name containing `_`; a raw id name never contains `_`, and folding preserves `_`. So such a derived name can never fold onto **any** legacy raw name.

Escaping only `a` would deliver (1) but not (2) — an `A`-containing id's name would stay identical to a raw name and could fold onto the *legacy* file of its case-pair twin, overwriting a tombstone that was never asked about. (2) is what makes a pre-existing corpus safe to write into, and it is why both members are escaped rather than one. It remains a **no-op for the ~60% of ids containing neither `a` nor `A`**, so most archive files keep the name they already had. (An id with neither character keeps its raw name, which is safe precisely because folding collides only on the `{A,a}` pair, so no *other* id can fold onto it either.)

**Collisions already on disk are a separate problem, handled separately.** The properties above stop the kit CREATING a folded collision; they cannot undo one a pre-fix version already wrote. So every archive resolution also **verifies the candidate file's own frontmatter id** before returning or deleting it. A file that positively claims a different id reads as not-found for the id you asked for (with a stderr warning), rather than handing back the wrong fact — and is never unlinked by a `cmk purge --hard` aimed at its neighbour, which matters because purge is irreversible and leaves no tombstone. A file with *no* parseable id is still returned, preserving the documented graceful-degrade contract for a garbled tombstone.

**Backward compatibility.** Reads try the derived name, then fall back to the **legacy raw-id** name; `cmk purge --hard` clears BOTH spellings. Existing corpora keep working with no migration, and nothing is renamed on disk — a rename would be a write on a read path, and renaming *into* a case-folding filesystem is the very hazard being fixed.

**The fork, and why this arm.** The alternative was dropping `a` from the alphabet. Rejected, in descending weight: (1) **it does not fix the bug** — every id already minted with an `a` stays on disk and stays exposed forever, and a durability fix that leaves deployed data broken is not a fix; (2) it breaks the alphabet's power-of-two property (32 symbols = exactly 5 bits/char, §3.1), i.e. it changes how ids are *derived* to fix how they are *stored*; (3) the collision is a property of the filename layer, not the id layer — the ids are distinct and correct.

**Prior art** (primary sources, 2026-08-13) splits on exactly this line, and both arms are right for their own precondition. git, rclone and Nix restrict the **alphabet** to a single case — git's `fill_loose_path()` emits loose-object names from a hardcoded lowercase hex table, so two object ids cannot differ only by case, and RFC 4648 §3.4 puts the case decision at the alphabet layer — but all three chose their alphabet at design time, before any data existed. (git's own case-insensitivity machinery, `core.ignoreCase` / `core.protectHFS` / `core.protectNTFS`, governs **working-tree paths**, never the object store, which has no such hazard by construction.) Mercurial's `.hg/store` **escapes** instead — `FOO` → `_f_o_o` — precisely because its names are already-deployed user-supplied paths it cannot re-alphabet, and its CaseFolding note states our exact problem. We are in Mercurial's position, not git's. Our requirement is in fact weaker than hg's: determinism, collision-freedom under case folding, and decodability, but no byte-exact round-trip of arbitrary input.

**Not a sanitizer.** The derivation maps `a` and passes every other byte through, including `.` `/` `\`. It neither adds nor removes path-traversal safety, so callers' existing `ID_PATTERN` gates remain the defense and must not be relaxed on account of it.
...
#### Detached children run in `tmpdir()`, never the working tree (Task 253c, 2026-07-28)

**The rule:** every `detached: true` + `unref()` spawn in the kit sets `cwd: tmpdir()`. A fire-and-forget child outlives its parent by design, and a live process holds an OS handle on its working directory — on Windows that handle blocks a later delete / move / rename of the directory it sits in. Pinning the cwd to the OS temp dir removes the kit's own children from that race entirely. Named technique borrowed from [obsidian-mind](../docs/research/2026-07-21-repo-obsidian-mind.md)'s detached re-index spawn (borrow candidate 6); it lands squarely on this project's documented Windows spawn-layer pain, of which the kit's own `EPERM`-at-teardown cleanups are an instance.

**Nothing depends on the inherited cwd — verified per site, both directions.** The project root travels by `argv` and/or `CMK_PROJECT_DIR`, and every path a child touches is absolute:

| Detached spawn site | Descriptor (the Door-3 seam) | How the child gets its root |
| --- | --- | --- |
| `capture-turn.mjs` → `cmk-auto-extract` (Stop hook) | `autoExtractSpawnDescriptor` | `CMK_PROJECT_DIR` env; both argv paths absolute |
| `inject-context.mjs` → `cmk-compress-lazy` (SessionStart, both branches incl. the `shell:true` corrupt-install fallback) | `lazyCompressSpawnDescriptor` | `CMK_PROJECT_DIR` env (`argv[2] ?? env ?? cwd` in the bin) |
| `cmk-precompact.mjs` (npm + plugin twins) → `cmk-precompact-worker` | `precompactWorkerSpawnDescriptor` | `argv[2]` **and** `CMK_PROJECT_DIR` env |

**The guard that makes it safe:** each child's last-resort root fallback is `process.cwd()`, so moving cwd to tmpdir *without* handing over a root would make it scaffold a memory tree in the temp dir. Every descriptor therefore pins tmpdir **only when a non-empty `projectRoot` is present** and otherwise inherits, exactly as before.

**Structural note:** the precompact shape moved OUT of the two bins into `precompact.mjs` as one shared descriptor. The npm and plugin bins are twins that each carried their own copy of the options object — which is how twins drift; a test now pins that both spawn through the shared descriptor. `validate-spawn-discipline` is unaffected: these are detached fire-and-forget sites and keep their documented `// spawn-discipline: ignore` markers.
...
**Where the flags come from, and why re-registering never helped.** They are `schtasks /Create`'s own defaults, written into every task the kit has registered since Task 33 — the starvation predates the catch-up work (see `lazy-compress.mjs`'s decision-trail comment on the v0.4.0 dogfood, where the cron never fired and `now.md` reached 410 KB). `schtasks /Create` has no CLI flag for any of these, so the kit applies them with a follow-up PowerShell `Set-ScheduledTask -Settings (New-ScheduledTaskSettingsSet …)` (added by Task 167.E, extended by Task 203). But `-Settings` **replaces the whole settings object**, and `New-ScheduledTaskSettingsSet`'s no-argument defaults ARE the hostile posture — verified empirically on Windows PowerShell 5.1 and against the schema's declared defaults (`DisallowStartIfOnBatteries`/`StopIfGoingOnBatteries`/`StopOnIdleEnd` all default `true`). So the call meant to make missed runs catch up **re-stamped** the battery flags and re-asserted `StopOnIdleEnd` on every registration: it did not introduce the starvation, it kept re-applying it.
...
- **Scan.** A pruned walk below the project root, bounded at **`MAX_SCAN_DEPTH = 4`** — the maximum number of INTERMEDIATE directories between the root and the tier, so `a/b/c/d/context` is found and `a/b/c/d/e/context` is not. A stray tier is created at the agent's cwd, and the realistic deep cwd is a monorepo package or a folder inside it (`packages/cli/src` = 3); this repo's two real strays were at 1 and 2. The walk skips `node_modules/`, every dot-directory, and the usual build/vendor/template trees, so a run is a handful of `readdir` calls — cheap enough for every install AND every `doctor`. **The unit is load-bearing and was wrong once:** an earlier boundary made the documented 4 find only 3, and nothing caught it because only the OVER-cap side had a test. Both sides are now pinned and the budget is registered in `validate-budget-pairs` (§17.10).
- **Discriminator (two questions, not one).** A candidate counts as a REAL stray only when it holds **live state** — a fact file, an INDEX.md that lists entries, a non-empty `sessions/now.md`, or another populated surface — AND its parent directory is **not a project in its own right**. The second half is not optional: a nested REAL repo, or the fat-finger case of running `cmk install` from a folder that holds several installed projects, presents a tier full of live state that is indistinguishable from a stray *when viewed from inside the tier*. The evidence has to come from the parent, and two markers settle it — a **`.git` entry** (dir OR file, so worktrees and submodules count: this directory is its own repository, so its `context/` travels with its own clone) or an **agent instruction file carrying the kit's `core-memory-kit:start` managed block** (positive proof it ran `cmk install` for itself). A neighbour is skipped by `scanStrayTiers` itself, so it is invisible to recovery AND to HC-13 — it is not "a stray we decline to fix", it is not a stray. This check **fails SAFE, not fail-open**: if it cannot tell, it assumes neighbour, because the cost of a false neighbour is one un-recovered stray the user can still move by hand, while the cost of a false stray is copying a live project's memory elsewhere and telling the user to delete the original. A `template/`-shaped scaffold has no live state and is likewise never touched.
...
- **Collision-safe.** A destination filename that already exists, or a fact id already present anywhere in the root tier, is SKIPPED and reported. Never overwrite, never dedupe-by-rewrite. The census covers the live fact dir plus **both** tombstone locations and the superseded archive — `<factDir>/archive/{tombstones,superseded,quarantine}/` AND `<tierRoot>/archive/tombstones/`, which is where `forget` puts a retired **scratchpad bullet** (a different directory, one level up — missing it let a forgotten bullet come back as a fact file). The archive half is load-bearing: copying a stray twin of a tombstone back into `memory/` would silently **resurrect a forget**, which would be a worse outcome than the stranding this fixes. The census reads each archived fact's id from its **frontmatter**, not from its filename, so it is unaffected by the §6.5 filename derivation — but it depends on every tombstone actually EXISTING as its own file, which is exactly what Task 281 / D-451 restored for a case-pair of ids.
...
- **Derivable → repaired.** The kit's ids are content-addressed (§3.1), so a missing id is recomputable from the body the file already holds: `generateId(tier, body)`, written back through the kit's own machinery. The edit is **textual, never a YAML round-trip** — an insertion after the opening `---` when there is no `id:` LINE at all, or a one-line swap that keeps the old value as a QUOTED `legacy_id` (so a citation of the old id still greps to the file, and a numeric legacy id round-trips as a string). Every other byte and the EOL style are preserved; the BOM deliberately is not (below).
  - **INSERT vs SWAP is decided by the LINE, not the parsed value.** `id:` with an empty value and `id: null` both parse to `null` exactly like a missing id, so keying off the value appended a SECOND `id:` line — a duplicate YAML key, which made the file unparseable, which the next run then quarantined. Corrupt, then evict, on the most likely wild shape there is.
  - **THE UNIVERSAL GUARD.** A textual edit against YAML the code does not fully model cannot be trusted on inspection, so the OUTPUT is verified before any write: the repaired text must re-parse, carry exactly the derived id, and still hold the same body. On failure the ORIGINAL bytes are kept and routed through the normal not-derivable → quarantine path — so an unanticipated shape costs the user a **recoverable original**, never a mangled file. This is deliberately a guard rather than another special case: the empty-`id:` bug was ONE unhandled shape, and the next one would have shipped the identical corrupt-then-evict outcome. Reference shapes it correctly rejects: a block-scalar `id: |` whose continuation lines cannot be spliced.
  - **The BOM is stripped on the repair write.** Normally this pass preserves every byte it does not own, but `frontmatter.parse`'s `^---` anchor does not tolerate a leading U+FEFF, so a BOM'd fact file is invisible to reindex, recall and every fact walk. Repairing the id while leaving the BOM would let the report claim "now indexed and recallable" about a file that still is not — so the pass, which already owns the rewrite, removes the one byte sequence standing between the fix and its own claim. (A BOM'd file with an ALREADY-VALID id is out of this pass's scope and stays invisible — the general `parse()` BOM tolerance is a separate, wider fix.) **Update (Task 257, D-403): that wider fix shipped** — `frontmatter.parse` now strips a leading BOM kit-wide (§4, "The input boundary"), so a BOM'd file with a valid id is no longer invisible and no longer depends on this pass noticing it. This repair path is unchanged and still correct: its strip is what keeps the **textual** id splice (which never goes through `parse`) BOM-aware, and dropping the BOM on a file it is already rewriting is the same heal-on-rewrite rule §4 now states as a contract. The local `stripBom` copy this module carried was replaced by the canonical `read-json.mjs` import in the same change.
...
**The HC-4 / HC-15 / HC-16 division is deliberate** — three checks over the index, each asking a question the others cannot: HC-4 compares INDEX.md's entry *count* against the file count (the committed markdown surface); HC-16 asks whether each fact *could ever be indexed* — running it through the indexer's own parser, not testing database membership, since the index rebuilds lazily on read and a just-written fact legitimately has no row; HC-15 asks whether an already-indexed fact's *vector* is its own (mapping). D-427 lived in the gap between them. (The two memsearch checks — formerly HC-1 "installed" + HC-7 "reachable" — were **removed in Task 120**; the remaining five from requirements.md renumbered to HC-1..HC-5, plus HC-6 native-memory detection per ADR-0011, HC-7 stale-lock detection per PR-B's class-2 lock audit, and HC-8 native-binding health per Task 141a / D-129. The cross-platform-emission audit lives in `validate-platform-commands.mjs`, not a runtime doctor check.)
...
**The gap.** The `references` family resolved every internal-id class the corpus uses — `ADR-NNNN`, `FR-N`, `NFR-N`, `Task N`, `§N.N`, file links — except `D-nnn`, which is the **most-cited** reference in the corpus (the DECISION-LOG is the chronological spine). The CLAUDE.md rule "internal cross-references are subject to the same primary-source verification" names the class, and `D-nnn` was its lone prose-only member. The concrete failures: `D-382` was cited in a task entry and a commit message *before the entry existed*, and the D-380..D-384 authoring hour cross-referenced four sibling entries — exactly the window where a typo'd `D-38X` resolves to nothing and nobody notices.

**The target case is the FORWARD reference:** `D-406` cited while the log stops at `D-405`. A typo that happens to land on an existing entry is out of reach of any anchor scheme — the check buys the dangling half, which is the half that actually happened.

**Anchor extraction — leads only, all ids in a lead.** `parseDecisionIds` indexes the D-ids appearing in an **entry lead**: a heading line, or the leading bold span of a list item. The log grew three lead shapes over 400 entries and all three are live (`## <date> — D-405 · TYPE — …`, `## <date> — D-375: TYPE — …`, `- **⚙️ DECISION (<date>) — D-150: …**`). Restricting to leads is what keeps the check's teeth: entry BODIES are dense with citations (`_Relates D-285, D-198._`), and indexing those would make every typo self-anchoring. Within a lead, **all** ids count, not just the first — `- **✅ FIX + RESOLUTION of D-212 (…) — D-213: …**` opens with a citation and carries its own id second, so first-only would drop a genuine anchor.

**The cost of "all ids in a lead", with both numbers** (they answer different questions, and quoting only one is the dishonest version):

| | Today's log |
| --- | --- |
| **Mechanism** — leads that *can* mint a false anchor | **110** (84 leads carrying more than one id + 30 indented sub-bullets that are formatting rather than entry leads; 4 are both) |
| **Outcome** — false anchors that mechanism actually produced | **1** (`D-552`, a typo inside D-270's lead — cited by nothing, so it masks nothing) |

The mechanism is broad, the realised cost is one. The trade follows the standing conservatism rule: an extra anchor can only MASK a citation; a missing anchor FAILS THE BUILD on correct prose. _Both figures move with the token — a review of this change measured 92 against the narrower `\bD-\d+\b`; widening for sub-letters and slash continuations in the same round moved it to 110. Re-measure after any token change rather than trusting either number._

**The citation TOKEN is mirrored from the kit's own parser.** `packages/cli/src/graph-index.mjs` (`ANCHOR_MATCHERS` + `slashNodes`, Task 232, same release) already parses D-anchors out of fact bodies, and it handles the shape this check first missed: **`D-270/271/277` is three citations**, so validating only the head silently exempted ~60 tails across the corpus. The token is mirrored rather than imported — that module pulls `fact-store` / `frontmatter` / `tier-paths` to build a graph table, and this validator is a zero-dependency lint step — with one deliberate divergence: an optional **sub-letter** (`\bD-(\d+[a-z]?)…`).

**Sub-lettered ids are checked on BOTH sides, because the log really anchors them.** `D-203b`…`D-203n`, `D-253a`, `D-258a` are 14 real entry ids in leads, and `CLAUDE.md` really cites `D-253a`. The alternative considered and rejected was to leave the token narrow and document the hole: that would half-check the class — `D-253a` unverifiable while `D-253` is verified — which is exactly the "answers with a subset of the truth" failure the verification rules name. Extending both sides costs nothing (the corpus resolves fully) and closes it.

**The anchor source is a GLOB, not a file — `docs/journey/DECISION-LOG*.md`.** Task 249 splits the append-only histories at a version boundary, putting the pre-split entries in `DECISION-LOG-archive-pre-v0.5.md` beside the live log. Reading one file would make every citation to a pre-split entry dangle **the day 249 lands** — the check would punish the repo for doing the archiving. The same treatment applies to `Task N`, whose ids now come from `specs/tasks*.md` (249 splits `tasks.md` into `tasks-archive.md` the same way). Sources are parsed **per file and unioned**, never concatenated, so fence state cannot bleed across an archive boundary. _(249 still owes the archives their frozen-record path prefixes — `docs/journey/` already covers the log archive; `specs/` does not, so `specs/tasks-archive.md` needs an explicit entry or its historical citations become policed.)_

**Fenced blocks and inline-code spans are stripped on BOTH sides.** The scan side always did; the parse side now does too. The asymmetry was real: a lead-shaped *example* inside a ` ``` ` block in the log would have minted an anchor, letting an illustration authorise a citation. Zero such examples exist today — the fix closes the shape, not a live bug.

**HTML comments are scanned, not stripped** — pre-existing behavior of the whole `references` family, stated here rather than left as a surprise. A `D-nnn` inside `<!-- … -->` is validated like any other, which is deliberate on balance: the suppression marker is itself an HTML comment on the same line, so a commented-out citation that is genuinely historical takes the marker like anything else.

**No source (or an empty one) ⇒ the sub-check is skipped**, and the summary says so rather than printing "0 indexed" — a check that did not run must not read like a check that found nothing. Turning one missing FILE into N citation errors is noise under a fixture root; deletion of the real log is caught by the `registry` family's direction 2 (`docs/journey/DECISION-LOG.md` is a backticked manifest entry).

**What the first run caught — 4 dangling citations, 2 distinct ids that were never numbered entries.** `D-132` (cited by `specs/tasks.md` Task 147 + `docs/RELEASE-PLAN.md`) was reserved when the log stood at D-131, and its content was **appended into D-133's entry tail** instead of getting its own numbered entry — backfilled as one, with D-133 left untouched. `D-168` (cited by `specs/tasks.md` Task 156, this document's §19.4.2 heading, and the build log) named a real entry that shipped **without its number** — the id was backfilled onto it. Both are the reserved-but-never-anchored class of D-129 and the ADR-0009/0010 backfills.

**Cross-references:** §17.7 (the family this sub-check joins), §17.12 (the frozen-record list it reuses), Task 249 (the archive split this composes with), Task 232 / `graph-index.mjs` (the token's source), D-249, D-129 (the backfill precedent). _Implements: Task 247._
...
### 20.8 The recall-trigger hybrid — gated cheap-index pointer hint + existence advertisement + skill-fire telemetry (Task 233, v0.6.3)

_Canonical HOW for [ADR-0024](../docs/adr/0024-recall-architecture-judgment-pulled-hybrid.md)'s build slice — keep judgment-pulled as the deep spine, close the under-fire risk (D-40/D-153) with three composed additions. Evidence: the [recall-trigger study](../docs/research/2026-07-19-recall-trigger-architecture-study.md) (pro-workflow hint + Letta advertisement) + Memora (the in-skill ladder refinements) + the [octopoda-OS code dive §5](../docs/research/2026-07-22-octopoda-os-code-dive.md) (the conservative relevance-floor warning)._

**1 — the gated cheap-index pointer hint (`capture-prompt.mjs::buildMemoryHint`).** The per-prompt hint upgrades from a static line to a REAL FTS5 query over the prompt's terms. Gates, in order: below `HINT_MIN_PROMPT_CHARS` (10) → `null` (no fire — "ok"/"go" never pay); no granular archive (`INDEX.md` with a `- (` entry) → `null`; prompt ≥ `HINT_QUERY_MIN_CHARS` (20) → run the query, else the static hint. The query is **recall-oriented, NOT the precision search**: it extracts content terms (drops a small stopword set + <3-char tokens, caps at 8) and OR-joins them — a whole natural-language question fed to `cmk search`'s per-token implicit-AND matches ~nothing (the doc would need every word, verbs and all). When the top hit clears the **bm25 score floor** (`HINT_BM25_SCORE_FLOOR = -0.5`; FTS5 bm25 is negative-better so a hit clears at `score <= FLOOR`) the hint injects up to 3 **index lines** — `id · title · date`, title from `INDEX.md`, **never bodies** — plus the skill pointer; below the floor / no hit → the static hint. The floor is **conservative (lenient)** per the octopoda-OS lesson (their 0.80 floor filtered 5/7 relevant facts): bm25 magnitude is CORPUS-DEPENDENT (measured — near-zero IDF on a 2-doc index, −4..−12 on a realistic corpus), so −0.5 cuts only near-zero degenerate single-common-term matches on a real corpus; it is a named, tunable constant (a real-corpus calibration pass may adjust it against the recall.log fire-rate). **Zero LLM/embedding.** **FAIL-OPEN**: this runs on EVERY prompt, so ANY error (missing index, sqlite failure) degrades to exactly today's static hint — never a crash, never a blocked prompt. The query opens the EXISTING index only (never reindexes — a slightly-stale index just misses a pointer; the DB is not created on the hot path, so a never-searched project stays static). This runs in the UserPromptSubmit hook — NOT the §20.3-pinned inject path — so the ADR's sanction of a real FTS5 query in the per-prompt hook applies; `openIndexDb` now closes-on-throw so a corrupt DB never leaks a handle. **Deliberate, measured trade:** this is the first time the interactive per-prompt path does a bounded SYNCHRONOUS sqlite read — accepted because it is measured (~8-9 ms on the real ~2000-fact corpus, latency-tested), fail-open, and opens the existing index only (no reindex, no DB creation on the hot path). **PRIVACY:** the prompt is screened at the top of `buildMemoryHint` (`sanitizePrivacyTags` + `maskPii`, FR-15) BEFORE the query or the log sees it, and the recall log stores only the extracted, screened, length-capped terms — never raw prompt text.

**2 — the existence advertisement (`inject-context.mjs::buildExistenceAdvertisement`).** The SessionStart snapshot gains ONE live metadata line — `Searchable memory beyond this snapshot: N recorded fact(s) · scopes: … · latest capture <date>` — so the model knows WHAT there is to search before deciding. **§20.3 IMPORT PIN honored:** computed WITHOUT opening the sqlite index (the inject path imports no index-db — a structural regression test pins the graph). The three inputs are all markdown/logs: the COUNT from `INDEX.md`'s `- (` entries (the reindex-maintained derived view); the SCOPES from tier-file presence (`facts` always; `decisions` when `DECISIONS.md` exists; `transcripts` when a transcripts/sessions dir holds markdown); the RECENCY as an absolute date from the audit-log tail's newest `created`/import-applied entry. **Byte-stable when memory is unchanged** (no `now`-relative rendering — the snapshot is prefix-cache-sensitive); reserved out of the cap like the preamble/temporal-mention/commit-proposal lines (§7.1.2), and emitted only with a body (empty-archive installs stay byte-identical to the pre-233 shape).

**3 — the in-skill Memora refinements (`memory-search` SKILL.md, both committed copies).** The ladder absorbs: (a) **score survivors 1–3, fetch bodies only when ≥2 survive at ≥2** (the policy-retriever discipline — filter hard, fetch little); (b) **episodic grouping** — hits sharing a `source_file` → ONE `expand`, not N `mk_get` bodies (restores the narrative the flat hits fragmented, at a fraction of the tokens); (c) the **relative-answer re-query** rule ("the same X as Sarah" → re-query the referent first, then X).

**4 — skill-invocation telemetry (the added scope — `recall-log.mjs`).** The kit logged recalls (`inject`/`search`) and extraction outcomes but NOT skill fires, so ADR-0024's own success criterion (fire-rate moves) was unmeasurable. Two derivable signals, both in the existing recall.log NDJSON: a **hint-fire** entry `{source:'hint', form:'static'|'evidence', ids, query, error?}` per fired hint (before/after: the evidence-vs-static share, and whether the hint precedes a recall) — `query` is the SCREENED extracted terms (never the raw prompt, capped), and `error:true` marks an errored evidence query so "errored" ≠ "no match" in the fire-rate data; a **search-origin** tag `{source:'search', origin:'skill', …}` written when `cmk search --source skill` / `mk_search {source:'skill'}` runs (the skill's first ladder step passes it), so a skill-driven recall is distinguishable from an ambient one. Both fields are added ONLY when set, so ordinary inject/search entries stay byte-shape identical and the Task-191/192 consumers (which filter on `source` = `search`/`inject`) are inert to a `hint` entry. Each entry also records the harness **tool-loading policy** (`toolPolicy`) from `$CMK_HARNESS_TOOL_POLICY` when the cut-gate exports it (P-DXPCKAUU: deferred-tools harnesses are a different fire-rate population). All appends best-effort (a broken diagnostic never breaks the hook).

**Explicitly out of scope (rejected in the ADR):** per-prompt SEMANTIC injection; inject-everything; the PreToolUse:Read file-context trigger (deferred until this slice's fire-rate numbers are in).
...
### 20.9 Source-aware injection split + the injection-size meter (Task 253, v0.6.x)

_Two independent riders on the SessionStart injection surface, borrowed as IDEAS from [obsidian-mind](../docs/research/2026-07-21-repo-obsidian-mind.md) (borrow candidates 1 + 3) and reimplemented in kit terms. Zero LLM, zero dependencies, fail-open everywhere. The third borrow (tmpdir-cwd for detached spawns) lands in §8.5._

**1 — the source-aware injection split (`inject-context.mjs::injectionModeForSource`).** Claude Code's SessionStart payload carries `source` — **primary-source verified 2026-07-27** against [code.claude.com/docs/en/hooks](https://code.claude.com/docs/en/hooks): *"How the session started: `startup` for new sessions, `resume` for resumed sessions, `clear` after `/clear`, `compact` after compaction, or `fork` for a new session forked from an existing one."* Both inject bins (npm + plugin) already parsed the payload for `session_id`; they now read `source` from the same parse and pass it through.

- **Pointer mode fires on `resume` ONLY** (`POINTER_MODE_SOURCES`). A resumed session replays its transcript, so the snapshot injected at session start is already in the model's context and re-injecting the tier bulk pays full byte price for nothing. The bulk is replaced by ONE byte-stable line (`SNAPSHOT_POINTER`).
- **The static/volatile split.** REPLACED: the tier blocks (`body`) and the state-label instruction that describes labels living inside them. STILL INJECTED: the authoritative-memory preamble (the D-40 under-fire class — re-anchoring authority is worth ~700 B), the existence advertisement (§20.8 — "what there is to search" matters *more* when the bulk is absent), and the volatile action lines (temporal mention + commit proposal — a resumed session has not seen the git dirtiness that accrued since). When in doubt, keep injecting.
- **`compact` DELIBERATELY KEEPS THE FULL SNAPSHOT — a narrowing of the borrow** (obsidian-mind pointers on both). Anthropic's own docs describe compaction as summarizing older history to free space ([docs/en/costs](https://code.claude.com/docs/en/costs)), so the pre-compaction snapshot survives as a *summary*, not verbatim text. Pointing at content that may have been summarized away would silently cost the session its memory — the one failure this kit exists to prevent. Compaction is precisely when re-anchoring earns its bytes.
- **Fail-open everywhere.** `startup` / `clear` / `fork` / an unknown value / a non-string / a missing or unparseable payload all take the FULL snapshot — byte-identical to the pre-253 shape. The cross-agent legs (Kiro / Cursor / Codex `inject`) pass no `source` and are therefore unchanged.
- **Two guards, and the optimization declines itself.** (1) **SIZE** — the pointer is taken only when it is genuinely *smaller* than the bulk it replaces. On a small corpus it is not: measured on the real bin, a one-bullet project emits **673 B on startup but 778 B on resume**, because the preserved preamble + advertisement + pointer line outweigh a two-line body — inverting the premise for exactly the fresh-install shape. (2) **CAP** — the pointer variant is not produced by `enforceCap`, so it is measured against `capBytes` and rejected under a pathological tiny cap in favour of the (always cap-honoring) full snapshot. An empty-memory project emits no pointer at all — the kit does not claim memory it has not got.
- **The mode label describes the EMISSION, never the request.** When either guard rejects, the result is a full injection and `injectionMode` / the status line / the `recall.log` entry all say `full`. This is load-bearing rather than cosmetic: the pointer-vs-full aggregation in the log is the evidence that the saving is real, and a label that reported the *requested* mode would count emissions that never happened.
- **Prefix-cache discipline:** the pointer line carries no count, clock, or path, so two resumes of unchanged memory are byte-identical.
- **Known bounded edge — memory written DURING the session, then resumed (documented, not silently accepted).** The pointer says "unchanged since session start", which is a statement about *the emission*, not a proof about the disk: a scratchpad bullet captured mid-session and then `--resume`d is not re-injected, so it stays invisible to that session's context. Three things bound it, which is why it ships as documented rather than gated: (i) the frozen-snapshot contract *already* says writes take effect at the next session start, and a resume continues the same session id; (ii) most capture lands in the granular archive, which the snapshot never carried — those facts are search-reachable in both modes; (iii) pointer mode still injects the **existence advertisement**, whose live fact count and latest-capture date DO move when something was captured, and the pointer line itself names `cmk search` as the reach. **Trigger to close it (D-248 form):** if a real session is observed missing a same-session capture across a resume, gate pointer mode on tier-file mtime vs. the last `inject` entry's timestamp for that session id (both artifacts already exist — a bounded `recall.log` tail read plus the stats the tier walk already does; no new sidecar state, per ADR-0002).
- **The sibling case — resuming a session that was NEVER injected into** (the kit was installed mid-session, the session predates the install, or its first SessionStart hook errored): the pointer then points at something that was never there. It is self-limiting rather than silent — the pointer line names `cmk search` as the reach, the existence advertisement still rides with the live fact count, and the very next non-`resume` start emits the full snapshot and heals it — so the worst case is one resumed session searching instead of reading. The same-session-capture trigger above closes this one too (a session with no prior `inject` entry is exactly what that lookup finds).
- **Why the `compact`-keeps-full rule makes the resume pointer SOUND, not just cautious** (the composition worth stating): the risk case for a pointer is "the context it points at was summarized away," and compaction is the only event that does that. Because a `compact` start re-injects the FULL snapshot, any post-compaction context already holds a fresh copy — so a `resume` arriving after a compaction points at memory that a compaction-time injection has re-anchored, not at text the summarizer ate. The two rules compose: `compact` restores, `resume` economizes.

**2 — the injection-size meter (`inject-context.mjs::buildStatusLine`).** Every SessionStart status line now ends with `· snapshot <N> B` (plus `(pointer)` when pointer mode produced it) — obsidian-mind's `formatInjectionSize`, kit-shaped.

- **Placement resolution (the self-reference question):** the meter rides `systemMessage`, the USER-display channel (D-130), **never `additionalContext`**. Three consequences, all load-bearing: the model pays zero tokens for it; the snapshot stays byte-identical so the prefix cache and every byte-stability contract are untouched; and there is no self-reference to resolve — it measures `snapshot`, a string it is not part of. Byte counts are therefore exact, not "pre-meter".
- **Door 5:** the same numbers land durably on the `recall.log` `inject` entry as `bytes` + `mode`, so the token saving is *measurable from the log* rather than asserted in prose. Both fields are added only when set, so `search` / `hint` entries keep their pre-253 shape.
- **Fail-open:** the meter is computed defensively and appended on every return path including the catch — a meter error degrades to no meter, never to a broken snapshot.
...
**The tail read is BOUNDED** — `HEALTH_TAIL_BYTES` (**64 KB**), one positional read, with the partial head line DISCARDED (a byte-slice of an earlier record would either fail to parse or, worse, parse into something wrong).

**Transition logging at the high-cadence sites — the composition that makes the bound safe (review finding B2).** The tail is bounded AND SHARED ACROSS ALL CLASSES, so a chatty writer evicts a quiet one. At one `ok` per MCP tool call and one per fact write, a few hundred healthy operations pushed a genuine two-strike streak of a SPARSE class off the window before the whisper ever read it — the sparse class was structurally **unreachable**, which is worse than noisy: the feature silently did not work for exactly the failures it exists to catch. The fix is at the source. `appendHealthTransition` keeps a per-process, per-class memory of the last outcome written and appends an `ok` only when the previous outcome was a fail — or unknown, so each process still records one healthy baseline. `fail` is never suppressed.

It is safe because it **preserves the streak semantics exactly**: consecutive fails stay adjacent (fails always write), and the `ok` that arrives *between* two fails always lands (the previous outcome was a fail) — so the transition that CLEARS a warning is never the one dropped. Only the 2nd..Nth consecutive `ok` disappears, and those carry nothing the 1st does not. The memory is a write-rate optimization, not state the verdict depends on: ADR-0002 still holds, because the active set is still derived from the log's own bytes. Applied at **`mcp-server`** (long-lived, hundreds of calls per session) and **`reindex`** (once per fact written); the low-cadence sites — inject, precompact, extract — append directly, since they run about once per session or per turn and their evidence should stay densest. **One subtlety worth stating:** a site that writes a `fail` for a class whose `ok` goes through the transition path must ALSO use the transition form, so the shared memory is updated — otherwise a stale `ok` in the map would suppress the real clearing `ok` and strand the warning. `writeFact`'s `index-drift` fail does exactly that.

The 64 KB bound is then defence-in-depth for whatever cadence a future site brings. The trade remains deliberate and stated: evidence older than the tail window is invisible to the whisper, which is correct — but with transition logging that window now spans a very long span of real activity, because a healthy class contributes one line per process rather than one per operation.

**`detail` is enforced, not just asked for.** It must match `DETAIL_TOKEN_PATTERN` (`/^[a-z0-9._:-]{1,64}$/i`); a violating value is REPLACED with `invalid-detail`, never written through and never thrown on (review finding I2). The prose contract said "never user content" and nothing checked it, so the first future site to pass `err.message` would have put stderr — which can carry a prompt, a path, or a secret — into a file the whisper reads. Losing the reason is acceptable; losing the event is not.

**The write is gated on the install marker** (`context/MEMORY.md`), not on a bare `context/` directory (review finding M2): `context/` is an ordinary folder name, and the kit must not drop a log into an unrelated repo's docs directory. `cmk install` scaffolds `MEMORY.md` on every project, so it is the honest is-the-kit-here test.
...
**Scope bound, recorded rather than assumed: the whisper is Claude-Code-only in wave 1.** The three cross-agent dispatchers (`kiro-hook-dispatch` / `cursor-hook-dispatch` / `codex-hook-dispatch`) call `capturePrompt` — the `<private>`-strip + transcript-append half — but have never emitted the per-prompt recall hint either, so there is no prompt-hook output surface on those legs to hang a whisper on. This is a pre-existing asymmetry the whisper inherits, NOT a regression. The consequence worth naming: the `troubleshooting` skill DOES ship to Kiro (`kiro-skills` copies all three skills), so a Kiro/Cursor/Codex user has the repair book but reaches it through `cmk doctor`'s **HC-14** rather than through a whisper. Closing it means giving those dispatchers a prompt-hook output channel, which is an adapter-parity task, not a health-nudge one. **Trigger (D-248):** the next cross-agent surface task, or the first report of a silent-capture failure on a non-Claude leg.

**Growth + concurrency.** The log has no runtime rotation, the `audit.log` posture (§16.13 covers both when it ships) — but its highest-frequency writer is the MCP `ok` append, one line per tool call, which is busier than anything in `recall.log`. At ~90 B/line and a realistic 10-100 tool calls per session that is single-digit KB/day in a gitignored tier; the bounded tail read means growth costs nothing at read time. **Rotation trigger:** if a real install's `health.log` crosses ~5 MB, it goes on the §16.13 rotation surface with `audit.log`. Concurrent appenders (the detached auto-extract child, the MCP server, the inject hook) are safe by the same reasoning as the sibling logs — `appendFileSync` opens `O_APPEND` and every line is far under `PIPE_BUF`, and the reader skips a torn line rather than failing on it.
...
**Where a class CLEARS is part of its contract, not an afterthought (review finding B1).** `index-drift`'s clearing `ok` lives in **`reindex()` itself** — the one function that rebuilds INDEX.md — so every route reaches it: the `cmk reindex` the whisper prescribes, `cmk repair --index`, the boot reindex, and `writeFact`'s inline rebuild. It was originally appended only at `writeFact`'s call site, which meant an agent that did exactly what the whisper told it to could not clear the warning, and it persisted for up to the 7-day window: the stuck-warning class §23.1 claims is structurally impossible, with the added insult that the kit prescribed the one command that could not help. **The general rule this generalizes to: for every registry entry, the `primaryAction` must be a command that actually writes that class's `ok`.** A test pins `index-drift`'s pairing so the two cannot drift apart.
...
**One composition worth naming, because it strengthens an existing documented edge:** §20.9 records a bounded case where a `resume` pointer points at a session that was never successfully injected into, and calls it self-limiting because nothing could observe it. An `inject-failing` streak is now exactly that observation — the case remains bounded, but it is no longer invisible.
...
- **The `troubleshooting` skill** — the third scaffolded skill, one keyed section per registry code (symptom → diagnosis → exact fix → fix class), with the confirm-first discipline stated ABOVE the first repair instruction and `cmk doctor` as the fallback section. It is tied to the registry BY TEST: every code must have a section, and every section must declare the same `fixClass` and `primaryAction` the registry declares — a repair book that drifts from the code producing the whisper would name codes the skill cannot look up, or repair at an authority the registry never granted. Kit failure codes only. **Its `allowed-tools` is exactly `Bash(cmk doctor)`, `Bash(cmk doctor *)`, `Bash(cmk reindex)`, `Bash(cmk reindex *)`, `Bash(cmk search *)`, `mcp__cmk__mk_search`** — diagnosis, the one silent-class fix, and read-only lookups. **No form of `cmk repair` or `cmk install` is granted** (review finding B3: the first cut granted `Bash(cmk repair *)`, which includes `--hooks` (rewrites `settings.json`) and `--format` (migrates COMMITTED memory markdown) — both confirm-class, and the section above claimed they were absent while they were not).

**Why repair is ungranted ENTIRELY rather than narrowed to `--index`:** a `Bash()` pattern is a PREFIX match, so `Bash(cmk repair --index*)` also admits `cmk repair --index --hooks`. The pattern language cannot express "this flag and no other", and a grant with a hole in it is worse than no grant, because the prose would be the only thing between a mis-fire and a rewritten settings file. Where the repair book says to propose a confirm-class fix, the skill proposes and the user runs. Two tests pin the boundary in both directions — the positive list matches exactly, and a blocklist asserts the absence of `repair`/`install`/`--hooks`/`--format`/`--all` — because the earlier "cannot mutate memory" test passed while the grant was wide open, having only looked for `cmk remember`-shaped verbs. _(A `validate-skill-sources` extension that checks every skill's grants against a per-skill allowlist is a candidate for the next skill-surface task, recorded in the Task 250 entry.)_
- **HC-14** — doctor's view of the same data, reusing `activeWarnings` rather than re-deriving the thresholds (a second copy would drift and produce "doctor says fine, the AI says broken"). Advisory (`warn`, never `fail`). Its repair line is offered only when it is a real next step (`cmk reindex`): most codes' `primaryAction` is `cmk doctor`, which is right in the WHISPER — where the model has not run doctor — and circular in a line doctor itself prints, so nothing is emitted there and the message's pointer to the troubleshooting skill carries the action instead. It **SKIPs rather than PASSes** when the log cannot be read: the reader is fail-open by contract, which is right on the hot path and a false green in a diagnostic whose job is to say what it could and could not verify.
...
6. **Manual refresh + per-view freshness labels** ("as of HH:MM" — the Pulse TabFreshness borrow). **SSE live-refresh shipped in Task 259** (§24.1.3); the manual control and the per-view label both survive it, because the stream can end and the page has to keep working when it does.
...
### 24.1.1 What wave 1 settled at BUILD time (Task 255, 2026-08-02)

The eight ratified points above are unchanged. These are the contract details the build had to decide and the design did not pre-specify — recorded here so the next reader finds one spec, not a spec plus a surprise.

- **Route scheme.** The HTML side is ONE page (§24.1 point 7) that routes client-side off `location.pathname`; the JSON side answers on the same paths. Three equivalent ways to ask for JSON, any of which is sufficient: the **`/api/` prefix** (`/api/facts`), a **`.json` suffix** (`/facts.json`), or **`Accept: application/json`** without `text/html`. Views: `/` ≡ `/facts` · `/fact/:id` · `/graph` · `/health` · `/decisions`. Anything else is 404 on both sides.
- **Read-only, enforced before routing.** Any method other than `GET`/`HEAD` returns **405 with `Allow: GET, HEAD`**, decided ahead of the route table so an added route cannot accidentally accept a write. No route maps a path segment onto a filesystem path — the only file read is the committed page, at a constant path — so traversal is unreachable rather than filtered.
- **Two hardenings the survey argued for and the ratified points did not name.** (a) A **`Host:`-header guard**: binding loopback does NOT stop DNS rebinding — a hostile page can resolve its own domain to `127.0.0.1` and read the user's memory through their browser — so a request whose `Host` is not a loopback literal gets **403**. (b) A **`default-src 'none'` CSP** on every response, which turns "no CDN, fully offline" from an author habit into a runtime guarantee (a stray external reference fails visibly instead of working on the author's machine). `frame-ancestors 'none'` is spelled out alongside it because that directive does **not** fall back to `default-src`; `X-Content-Type-Options: nosniff` and `Referrer-Policy: no-referrer` ride every response too.
- **XSS defense is the RENDERING discipline, not the CSP.** The page renders untrusted content — fact bodies, FTS snippets, health messages — and is safe because it never parses HTML: every write goes through `textContent`/`createElement`/`append`, and the FTS `<b>` markers are re-created as real elements rather than interpolated. Because that is a discipline one careless edit could undo, a test **scans the inline script for the HTML-parsing sinks** (`innerHTML` / `outerHTML` / `insertAdjacentHTML` / `document.write` / `srcdoc` / `setHTML` / `createContextualFragment`) and fails on any occurrence. The CSP is a second line with a *different* job (offline-ness + anti-framing), not the XSS guard.
- **Port + lifecycle.** `--port` defaults to `0` (OS-assigned); an in-use port fails with a stated reason, not a stack. `--host` accepts only loopback literals. `--no-open` suppresses the browser; the URL is always printed. `SIGINT`/`SIGTERM` close the server and exit.
- **Freshness.** The three DB-backed routes (`facts` / `fact` / `graph`) re-run the same incremental boot reindex every other read verb runs; `health` and `decisions` read their sources directly and never touch the index. The page labels each view "as of HH:MM" and offers a manual Refresh. The DB handle is opened and closed **per request** — a long-lived handle would keep a Windows lock on `context/.index/` (the D-302 half-broken-install class) for as long as the tab is open.
- **Two budgets** (both registered in `validate-budget-pairs`): `VIEWER_DEFAULT_LIMIT=50` / `VIEWER_MAX_LIMIT=200` for the facts list — an over-cap `?limit=` **clamps** and reports `clamped:true` rather than 400ing a URL someone typed; and the graph's `node_limit`, which budgets **live facts only** — anchor hubs, dangling link targets, and archived-superseded predecessors ride on top and are each counted separately (`anchor_count` / `dangling_count` / `archived_count`), because a supersession arrow with no tail is not a supersession arrow, and a broken link is not a hub. Revisit trigger: a corpus where their sum approaches `node_limit`.
- **A budget states its own denominator (Task 268).** `/api/facts` reports `count` (rows in THIS response) and **`total`** (rows the query matched, unlimited) in both modes — browse counts the live corpus under the same `deleted_at` / `expires_at` / tier filters as the row query, search counts the whole FTS match set via the shared `countKeywordMatches` (same MATCH expression, same filters, same Task-211 state-view rewrite — a caller-side COUNT of the raw query would legitimately come out *smaller* than the page it is the total of). Without it the page had to source its headline figure from `/api/graph`'s `fact_count`, which at the time counted a DIFFERENT population (it filtered `deleted_at` only) and costs a COUNT + an edge SELECT + an archived-fact fs walk to paint one number; in search mode it had nothing to state but `count`, so a 500-hit query rendered "50 matches" in the largest type on the page. A number the UI promotes is a number the payload owes it. **And the corpus has ONE definition across the routes (2026-08-08):** `/api/graph` now filters `deleted_at IS NULL AND (expires_at IS NULL OR expires_at > now)` in *both* its node query and its COUNT, off the same `now` — so the graph rail's 56px "facts on disk" figure and the facts view's 56px total cannot disagree, and the budget's `nodes(live facts) === fact_count` invariant survives a corpus that has expired facts in it. Counting a population the graph does not draw would have replaced one disagreement with another.
- **A budget that hides records owes a way to REACH them — `?offset=` (Task 269, D-426).** Wave 1 shipped `?limit=` and nothing else, so `/api/facts` and `/api/decisions` answered the first ≤200 records and **no URL existed for the 201st**: 91% of a real fact corpus and 92% of the decision journal were unreachable, and `/api/decisions` said so in its own envelope (`{count: 200, total: 2502, truncated: true}`) while offering nothing to do about it. Both routes now take **`?offset=`** (non-negative integer; absent/empty = 0), and the envelope states the position rather than implying it: **`offset`** (the window's start), **`count`** (rows here), **`total`** (rows the query matched), **`reachable`** (how much of `total` paging can actually get to) and **`has_more`** (`offset + count < reachable` — the pager's whole question, answered by the payload instead of guessed from `count === limit`, which is wrong exactly once: on a corpus whose size is a multiple of the page size, where it offers a Next that leads nowhere).
  - **OFFSET, not a cursor** — decided at build. A cursor is the better answer for an unbounded feed; this is a bounded archive read deliberately, and three properties settle it: the browse sort is already a **total order** (`created_at DESC, id ASC` — the id tiebreak is what makes a same-millisecond write batch pageable at all), a **ranked** result set has no stable key a cursor could carry (the trust blend re-ranks in JS), and only an offset can say "records 101–150 of 2,300" — a **citeable** position, which is the point of a pager over an infinite scroll. The cost, stated: a write DURING paging shifts a newest-first window by one, so a record can repeat or be skipped across a live insert. On an ephemeral read-only viewer over a corpus that changes at human pace that is the right trade; a cursor is the fix if it stops being.
  - **A ranked page is a SLICE of one ranking, and the ask is a CONSTANT DEPTH — not `offset + limit`.** Every search page asks the engine for exactly `VIEWER_SEARCH_DEPTH` rows and slices the result. Asking for `offset + limit` looks equivalent and is not, and the difference is a data-loss bug rather than a ranking nicety: the engine **oversamples** (`BLEND_OVERSAMPLE × requested`) and re-ranks the candidate window in JS, so a 50-row page ranks a 150-row pool and a 150-row page a 450-row one. A trust-blended fact sitting past the smaller pool is invisible to the early page and present for the later one, and it shifts every row after it — reproduced on a 260-hit corpus as **260 rows walked, 249 distinct** (eleven ids on two pages, eleven on none), with the TOP hit changing between pages, which silently corrupts the `relevance` normalization that is defined against it. A constant ask pins the pool at the engine ceiling, so every page slices the identical ranking. **Cost:** a full-depth search per page — bounded, human-paced, and the price of a pager that cannot lose a record. Pushing the offset into SQL instead would have the same defect for the same reason. The price of the whole approach is a **reachable depth**: `reachable = min(total, VIEWER_SEARCH_DEPTH)`, where that constant IS `search.mjs`'s own `SEARCH_MAX_LIMIT`, imported rather than copied so the two cannot drift. Past it the route answers an empty 200 with `has_more: false` and the page says "paging reaches the first N — narrow the search for the rest." Browse mode has no such cap (a plain stable `ORDER BY` pages to the last record), so `reachable === total` there. **The beyond-depth page still calls the engine, at its smallest legal size (one row, discarded)** — so both branches reject identically instead of the unlimited count raising as a 500 what a normal page would have refused as a 400. That is defensive: `prepareFtsQuery` sanitizes every user query into a quoted term, so no `?q=` value is known to reach FTS5's parser raw.
  - **`has_more` counts the WINDOW, not the rows returned.** An FTS hit whose `observations` row cannot be resolved is dropped from the response (the Task-270 orphan class), so `count` can be short of the window that produced it. Deriving `has_more` from `count` would end the pager one page early on exactly the corpus where a record is already hard to reach, and the page's Next advances by `limit` for the same reason — stepping by `count` would re-serve the window it just showed.
  - **An over-cap `limit` clamps; a malformed `offset` 400s.** Not an inconsistency: an over-cap limit has an obvious sane reading ("give me the most you will"), and an offset past the end has one too — an empty page, served as a 200 that still states `total`. What is left (negative, fractional, non-numeric) has no reading at all. **The ceiling is `Number.MAX_SAFE_INTEGER`, and above it the verdict is 400, not a clamp:** a 20-digit `?offset=` parses to a float that is no longer the number that was typed, and reached SQLite's int64 bind as a **500 with a stack** — on the very route whose contract promises a 200 past the end. Above 2^53 an offset is malformed, not large; every offset that is genuinely a number, including every one past the end of the corpus, still gets its empty 200.
  - **`/api/decisions` gained `?q=` and `?order=`.** The journal is 2,502 entries, so searching it is the only realistic way in: `?q=` is a case-insensitive substring match through **`matchDecisionEntries`** — the SAME matcher `cmk search --scope decisions` and `mk_search` run (search.mjs owns it; a UI matching by its own rule would be a second search engine over one file, and the two would disagree the first time either was touched). Filter → order → page, in that order, so `total` is the match set and the pages partition it. **`?order=` defaults to `journal`** (oldest→newest, the order the kit appends in and the chronological trail this view exists to show — the shipped contract, unchanged); **`order=newest` is what the PAGE requests**, because page 1 of a real journal in append order is its OLDEST 50 decisions and the tab has always been labelled "newest first". `truncated` keeps its meaning — are there matching entries this response does not carry — now asked of the match set rather than of the whole file.
  - **`relevance` + `relevance_band` on a fact row (the D-429 sub-item).** FTS5's bm25 rank is unbounded, negative, and meaningless across two different queries, so there is no absolute 0..1 score to report. `relevance` is therefore **relative**: a hit's blended score over the **top** hit's blended score for the same query, in (0, 1], with the top hit 1.0 by construction — it answers "how much weaker than the best match is this", which is the question a colour on a result row can honestly answer. The reference point is the top of the WHOLE ranking, taken **before** the page slice: normalizing per page would repaint every page's first row as full strength, which is precisely the "a weak match looks strong" defect this closes. Bands are `strong` ≥ 0.7, `fair` ≥ 0.4, else `weak` (mnemory's thresholds). Both fields are **`null` in browse mode** — a browse row has no query and therefore no relevance to claim — and stay PRESENT rather than omitted, because a search hit and a browse row are one shape on the wire (the page must not have two renderers). The **decisions** scope gets no relevance at all: its `score` is a journal POSITION, not a rank, and colouring it would be a number pretending to be a different number.
- **The viewer writes NO kit log.** The facts route calls the shared `search()` **without** `projectRoot`, which is what suppresses its recall-log append: a human scrolling a UI is not the model recalling a fact, and browse traffic in `.locks/recall.log` would corrupt the Task-233/ADR-0024 fire-rate measurement. The only thing a request touches is the rebuildable `context/.index/` cache.
- **No threshold is re-derived.** `/api/health` calls `runDoctor` + `activeWarnings` (§23) directly, so the pinned strip and the per-prompt whisper cannot disagree about whether memory is working. `?strip=1` answers the pinned line ALONE (health-log tail + the two queue counts — all cheap file reads) and skips the doctor: the strip is on every view, so paying the full doctor check list including a subprocess probe per navigation would make the viewer feel broken.
- **The strip never claims more than its inputs support.** The health LOG records failures of things that RAN; a hook that was never registered never runs and therefore never logs, so a log-only verdict prints a green all-clear over a red doctor table (the HC-10 false-green class, on the UI's most prominent line). So: **on the full route the doctor results are folded in** — any `fail` outranks everything and the line names the failing check ids, a `warn`-only table reads amber; **on `?strip=1` the positive wording is bounded to what the log proves** ("no failures recorded (quick check)"), never a positive claim about capture/recall/index. Precedence, highest first: doctor FAIL → active health-log warning → doctor WARN → **an unreadable queue** (unknown ≠ zero — a queue read that throws reports amber, never a silent green) → pending queue items → all-clear. The health VIEW repaints the pinned line from its doctor-informed strip so the two can't contradict each other on screen.
- **The strip carries the queue count** (§24.1 point 8's stated answer to the deferred conflict-queue UI): with nothing failing but a non-empty queue, the strip reads `queued` and names the resolving verb (`cmk queue conflicts` / `cmk queue review`). The counts stay in the payload at every severity.
- **Actions render only where they work.** A LIVE fact's copy commands must be real AS PASTED: `cmk forget <id> --yes` (a bare form stops at a prompt, so the paste appeared to do nothing — the human choosing to paste and run in their own shell IS the ADR-0018 confirmation), and trust is a **template** over the whole ladder with the current level marked and no command offered for it, because the previous single line echoed the current level and changed nothing. An **archived-superseded** fact has left the live corpus, so `forget`/`trust` are no-ops against it: it carries `commands: null` plus a `state_note` naming its successor instead of three controls that look actionable and are not. The fact payload's `from_archive` flag says where the row was read from — deliberately NOT named `superseded`, which on a graph node means something else ("has a successor").
- **Superseded facts are reachable.** They were MOVED out of the live corpus and so are not in the index — but they are exactly what a supersession arrow points at, so both the graph and `/api/fact/:id` read them from `archive/superseded/` via the shared `eachSupersededFact` walk.

### 24.1.2 The visual contract (Task 260, 2026-08-03 — the visual pass, D-420; Task 268, 2026-08-06 — the redesign, D-425; direction revised 2026-08-08 to dark-default, D-432, completed the same day with the reader-owned theme toggle)

Wave 1 shipped with the brief "legibility over polish", which was never a ratified criterion, and the page read as undesigned. The pass is CSS + markup only — no route, no payload, no state semantics moved. The rules below are what the next editor has to keep; the values come from the [visual-design survey](../docs/research/2026-08-03-viewer-visual-design.md), not from taste. **The zero-dependency constraint (§24.1 point 7) is unchanged and was never the cause** — Deno's docs ship 117 KB of CSS with zero drop-shadows and look good.

- **THE DIRECTION. Original position (ratified 2026-08-04, memo §3 — preserved as the decision trail):** *"an instrument panel over a warm archive"* — everything you READ is a warm **light** document (paper ground, rows with shared boundaries, one display tier), and exactly ONE region is permanently dark and permanently saturated: the graph, the hero object, keeping its own `--g-*` colour space in both themes. It was chosen over dark-first on the reasoning that the dullness was polarity-invariant, that this is a reading surface, and that warm-light-plus-one-dark-canvas is a position nobody in the reference class holds.
  **Revision 2026-08-08 (D-432 — revisits the above with new evidence: three consecutive dark-preference verdicts from the user, ending in a commissioned dark pass):** **dark is the DEFAULT; light stays fully supported.** The product lives beside a terminal and dies with it (Ctrl-C), so it is built out of the same material — warm near-black under warm bone ink, and **exactly ONE accent (amber), drawn from the ground's own hue family** rather than the cold violet that was a second colour system laid over a warm page. The graph is still the hero, but by a different mechanism: it is **deeper than the page** and holds the only dense saturation in the product, instead of being the only dark thing on it. What did NOT change: rows-not-cards, the display tier, the corpus-stating overview, the viewport-sized instrument, the `--g-*` space being identical in both themes, and every rule below.
- **One token block per theme, and both are DESIGNED — neither is an inversion of the other.** A warm neutral ladder — `--ground` / `--panel` / `--sunken` / `--line` / `--line-soft` and three inks — redefined in full for the other theme rather than inverted. The page and the panel must differ by a real step: the shipped v1 was `#fbfaf8` on `#ffffff`, a 2% delta, which left 1px borders doing 100% of the separation. **Since D-432 the DEFAULT block (`:root`) is dark.** Tier hue-coding is a **light-only** encoding: with one accent that doubles as `--warn`, dark has no hue budget to spend three ways, so its tier glyphs are monochrome and the LETTER carries the meaning.
  **The reader chooses the theme, and nothing else does — the D-432 completion (Task 268 rider).** D-432's first cut expressed "dark is the default" as `:root` dark under a `@media (prefers-color-scheme: light)` override, which handed the decision straight back to the operating system: on a light-preferring machine the page was byte-identical to the one before the redesign, and the maintainer's verdict on the shipped result was *"looks the same to me"*. The `[data-theme]` hooks existed but nothing stamped the attribute, so the toggle they were written for did not exist either. **Now:** an inline script in `<head>` — before the first paint, so there is no flash — stamps `data-theme` on `<html>` from `localStorage['cmk-viewer-theme']`, falling back to **`dark`**, and **never reading the system preference**; a **button in the header chrome** (beside the read-only pill, labelled with the WORD for the theme the click moves to, `aria-label` stating the action) flips it and persists it. Persistence is best-effort and the flip is not: a blocked-storage profile still gets the theme it clicked, for that session. **The sheet holds exactly TWO root blocks — `:root` (dark) and `:root[data-theme="light"]` — each palette written ONCE.** The media block is deleted rather than left as a dead layer, because CSS asserting "the OS decides" is CSS stating the policy this reverses; `[data-theme="dark"]` is deleted because `:root` already is dark. This retires the byte-identical-duplicates rule the first cut needed (four copies, pinned copy-to-copy so a copy could not drift out of AA coverage) and replaces it with a stronger one: **the light override's token SET is checked against the dark palette itself** — so a token added to `:root` and forgotten in light fails, which two agreeing copies could not catch — and every theme-varying token must appear **exactly twice** in the sheet, which is what keeps a third copy from coming back.
- **ONE badge rule, and it is a TINT.** Every pill — trust, tier, state, health status, the graph legend, the active tab — is `background: rgba(<hue>, var(--tint))` under `border: 1px solid rgba(<hue>, var(--tint-line))` with the text in the hue; a variant sets `--hue`/`--hue-rgb` and nothing else. Hues carry their channels as `--*-rgb` so the tint is a **literal rgba pair** — deliberately not a colour-mixing function, whose support the survey could not verify and which both reference implementations avoid; a test pins every `--*-rgb` to the channels of its own hex, because the two halves of a hue are only related by a human keeping them in step. One hue per meaning, unchanged from wave 1 — **with one ratified exception (D-432): in dark, amber is both `--accent` and `--warn`**, because a one-accent palette drawn from the ground's own hue family has no second warm to spend, and the two are disambiguated by SHAPE rather than hue (an accent is a pill, a warning is the full-width bar).
- **The fact list is ROWS in one bounded surface, under a real DISPLAY tier** (Task 268 — the wave-2 redesign; the wave-1 shape was fifty floating cards and no type above 16px). Three display steps sit above the UI text — **56px** (the corpus figure) · **30px** (view title) · **20px** (record title) — each with a tracking ramp that tightens as it grows, over a `--measure` reading column (**720px**). Inside a row the tiers are title (14.5/600) · snippet (13.5, second ink) · meta (12, third ink), with the tier glyph in a fixed column so it reads as a vertical rail rather than a pill that moves with the text. The server still sends ONE flattened string per fact: **the title/body split is a client concern** (a line break first, then a sentence end, then dash/colon, then a clause break, then a word break), and the overflow is a CSS `-webkit-line-clamp` rather than more server-side `…`. **The clamp is lossless — the marker strip is not**: clamped text is only visually hidden and stays in the DOM and the accessibility tree, but the LIST mode additionally strips list bullets and heading hashes before rendering, so those characters are genuinely gone from that view. The detail view keeps them (next bullet).
- **List mode and detail mode are opposites, and that is the point.** A list snippet is already flattened to one line by the server, so a leading `- ` is noise and markers are stripped; the DETAIL view renders the record's real multi-line body, keeps every marker and every line break (`white-space: pre-wrap`), and clamps nothing — matching `richBody`, which renders Why/How two blocks lower on the same page. Rendering one page both ways was the Task-260 review's Blocking finding: a 7-line bullet headline came out as one merged title over six unmarked lines.
- **Contrast is a contract, not a vibe: WCAG AA — 4.5:1 for text, 3:1 for UI and focus indicators, in BOTH themes.** Body text must clear 4.5:1 on *every* surface it can land on (`--panel` / `--ground` / `--sunken`), and a badge must clear 4.5:1 against **its own tint composited over both `--panel` and `--ground`** — a badge is not text on a surface, it is text on a wash of itself, and it appears on cards *and* on the page (the graph legend, the health rows). Small bold text does not earn the 3:1 large-text exemption: 11px/600 is body weight for WCAG purposes. A test computes every pair from the served CSS and fails the suite under target, so a token cannot be lightened to taste — the first cut of this pass shipped 15 failing light pairs, the worst at 2.93:1.
- **The surface ladder is ORDERED, not merely distinct.** Light recedes by darkening (`--panel` > `--ground` > `--sunken`); dark recedes by darkening too (`--panel` > `--sunken` > `--ground`). A `--sunken` lighter than `--ground` made the graph canvas and the code chips read as raised. Card hover uses `--panel-hover`, a one-step tint, because a genuinely recessed `--sunken` is too dark for it.
- **The row rhythm: real internal padding, and ONE shared hairline.** A row carries **≥10px** of vertical padding and separates from its neighbour with a single `border-top` inside a surface that clips them (`overflow: hidden`), so N records read as one object with N entries. **Original rule (pre-Task-268): "card padding EXCEEDS card gap"** — it was 13px inside a 9px gap, so cards crowded each other harder than their own contents. That failure is now structurally impossible: rows share edges, so there is no inter-card gap left to invert, and the surviving contract is the padding floor plus the shared boundary.
- **The health strip's SHAPE is earned; its text and state semantics are the server's** (§24.1.1's precedence ladder, unchanged). `ok` and `queued` are an inline **pill**; the full-width **bar** is reserved for `warn` and `bad`. Pending queue items sit one rung above all-clear, so giving them the loudest shape on the page would repeat the bug this pass fixed (a full-width pale-green bar for the GOOD state) in a new colour.
- **The graph is ONE dark instrument, and it sizes to the WINDOW** (Task 268). Plot and rail sit on one canvas in the `--g-*` colour space, which does NOT flip with the system theme — an object does not change material when the room lights change. **Since D-432 that canvas is DEEPER than `--ground` in both themes** (`#131110` plot / `#0f0d0c` rail): with the page itself dark by default, the instrument can no longer read as an instrument by being the only dark region, so it earns the distinction by depth and by holding the only dense saturation in the product. The box is viewport-relative (`dvh` with a `vh` fallback, subtracting the chrome above it, floored at 440px) rather than a fixed height that started the hero below the fold on a laptop; the subtraction is calibrated against the health strip's QUIET pill, so the loud warn/bad bar earns its own larger subtraction via `:has()` (an engine without `:has()` degrades to a scrollbar in warn state only). The rail carries the corpus count, the click-to-isolate cluster legend, the static "reading it" key, and — only while something is pointed at — a record peek, inserted ABOVE the static key because live feedback that lands below the rail's fold is no feedback. Below the stacked breakpoint the instrument opts out to `height: auto`: there the plot and rail are siblings in one column and a viewport-height box would cramp both.
- **The graph draws only what has a relationship.** Degree-0 nodes are dropped before layout — the force layout could only push them to the wall, producing a picture-frame of ~150 orphan dots — and **counted in the note** (`N unlinked, not drawn`), never hidden silently. Standing labels are earned at degree ≥ 3 (anchors always), truncated to 30 chars (`LABEL_CHARS`), halo'd with `paint-order: stroke` in the canvas colour, and every other label is drawn hidden and revealed on hover/focus. Supersession, the one directed claim on the canvas, draws at full stroke-opacity while association recedes to texture.
- **The pager is PREV/NEXT over a stable sort, and there is ONE search box for the product (Task 269).** A memory archive is navigated deliberately, so the affordance is a pager and never an infinite scroll: an infinite list has no URL for "the page the fact I mean is on", and an unciteable position is the wrong property for the one surface whose job is showing you what you decided. Both controls are **real anchors** carrying the whole view state (`?q=` / `?tier=` / `?offset=`), so middle-click, copy-link and the back button all work; an unavailable direction renders as a **span**, never a dead link; and the range line states `Showing 51–100 of 2,300` in tabular figures, plus the reachable-depth sentence when a ranked set is deeper than paging can go. The pager REPLACED the "showing the first 50 of 2,300" note, which stated the shortfall and offered nothing to do about it — the D-426 finding one layer up. **View state lives in the URL, and the controls follow it, never the reverse** — a deep link, a back button and a nav click must all paint the query the box is showing. Typing writes with `replaceState` (one history entry per keystroke would bury the page the reader came from) and **always resets the offset to 0**; a pager click uses `pushState`. **One `<input type="search">` exists in the whole page** and is MOVED into whichever view is showing (a `.tb-slot` per searchable view; appending an existing node relocates it) rather than duplicated — two inputs mean two debounces, two states, and a second one that is always the one out of sync with the URL. The tier select is a FACTS concept and is hidden on the journal rather than left there filtering nothing.
- **A relevance band is a WORD first and a hue second.** `strong match` / `fair match` / `weak match` render as the standard tint pill, coloured from the kit's existing good→poor ladder (`--ok` / `--warn` / `--bad` — already AA-tested on both surfaces in both themes), with the exact figure and its definition in the `title`. Same discipline as the graph's trust rim-arc: **colour is never the sole encoding**, so this survives every kind of colour blindness and a greyscale print. It is a third reading of a ladder trust levels and health statuses already use — not a fourth colour system, which a dark theme with one accent has no budget for. The page renders the SERVER's band and re-derives no threshold, and the class name is read out of a **`Map`** keyed by the three bands — not an object literal gated on truthiness, which inherits a prototype chain and would let a band of `constructor` past the gate and then interpolate the payload's own string into a class attribute. The lookup is closed by construction rather than by a comment saying it is.
- **Markdown in a snippet is TOKENIZED, never parsed.** `**bold**` rendering literally was a real defect; the fix is a tokenizer over plain strings that emits `code`/`strong`/`b` elements built with `createElement` + `textContent`. **The §24.1.1 XSS discipline is unchanged and is what forbids the obvious alternative** — the sink scan still fails the suite on any HTML-parsing sink. The `§24.1 point 7` zero-dependency rule is also why the page has no DOM test environment, so the pass's own tests are structural assertions over the served bytes and the RENDERED result is verified by a human with a browser — stated rather than implied.
...
- **The mechanism is POLLED-FROM-ARTIFACTS, not watched — and that reverses the obvious choice on evidence, not taste.** The route holds a `text/event-stream` open; a per-server interval `stat`s a fixed list of tier artifacts every **2 s** (`VIEWER_POLL_MS`), hashes them, and emits an event when the digest moves. **`fs.watch`/chokidar was the expected answer and was rejected**: D-329 (Task 218) measured chokidar **dropping events 5/5 under full-suite load** on the always-busy-laptop case and its 17-project survey found the field re-reads per request rather than trusting a watcher — a change-detector that silently misses changes is strictly worse than none on a surface whose whole claim is freshness. It is also the one platform-sensitive moving part §24.2 kept out of this server. The task entry anticipated this ("evaluate polling-mtime as the watch mechanism before fs.watch"); the evaluation came out the way the entry hinted, so the §24.2 exclusion is now permanent rather than deferred.
- **What gets `stat`ed, and why it is O(1) in the corpus.** Per tier (P/L/U): **`.locks/audit.log`**, the **fact dir's own mtime**, every scratchpad in **`SCRATCHPADS_BY_TIER`** (not a hardcoded `MEMORY.md` — the Task-182 lesson that the persona files are scratchpads too), and **`DECISIONS.md`** on P. The audit log is the load-bearing one: CLAUDE.md's shared-module rule makes `appendAuditEntry` mandatory for every mutating kit operation, so it is the closest thing the kit has to a transaction log and it is what catches **in-place** change — a `trust` edit, a `forget` tombstone, a supersession — which no directory mtime ever shows. The fact-dir mtime covers the complement: an entry added or removed by something outside the kit (a `git pull`, a checkout, a second agent). **The two are complementary — and the pair is NOT exhaustive, which is worth stating rather than implying.** Between them they cover every change the KIT makes (all of which audit) plus external add/remove. What nothing here sees is an external **in-place** edit — a hand-edited fact file, or a `git checkout` that rewrites a file's bytes without touching the directory entry. That is an **accepted gap**, not an oversight: hand-editing memory files is forbidden (the safe write path is the only sanctioned writer), and closing it would cost a content hash of the whole corpus every cycle, which is exactly the O(corpus) poll this design exists to avoid. Refresh remains for the reader who does it anyway. **The `queues/` files are watched too** (Task 259 I1, review + conflicts, named individually so an append to an existing queue moves the fingerprint): the pinned health strip on every view reports their pending counts, so a queue growing while the strip reads "all clear" would be this feature failing on the most prominent line of the page. A recursive walk of `context/memory/` is the obvious alternative and is the wrong shape — O(corpus) every 2 s, forever, on a corpus already in the thousands, to detect an event that happens a few times an hour. Each stat contributes `mtimeMs` **and size**, because mtime resolution is coarse on some filesystems (FAT 2 s; some network mounts round to the second) and an append landing inside one tick would otherwise be invisible. A missing path contributes a constant rather than being skipped, so a file *appearing* is a change and so is one being deleted. The digest is **hashed** before it goes on the wire: the input is a list of absolute paths including the user's home directory, and the home-path rule that governs what the kit writes to disk governs what it puts on a socket.
- **The stream is a NOTIFICATION, never a payload.** A `change` carries `{fingerprint, generated_at}` and nothing else; the page re-fetches through the same API every other read uses. Shipping rows down the stream would mean a second read path that has to know which view is open, which query it is under and which page it is on — two renderings of one envelope, and the one that drifts is the one nobody is looking at. Frames: **`retry:`** first (`VIEWER_RETRY_MS`, stated rather than left to the browser's implementation-defined default, because the page's give-up budget is counted in these intervals), then **`hello`** `{stream, poll_ms, heartbeat_ms, fingerprint, generated_at}`, then `change` as they occur, with a `: ping` comment every **15 s** (`VIEWER_HEARTBEAT_MS`) so a quiet stream is distinguishable from a dead one.
- **`/events` is a view with exactly ONE rendering, and that branch is load-bearing.** `resolveRoute` picks HTML-vs-JSON from `Accept`, and an EventSource sends `Accept: text/event-stream` — **neither**, so it would fall to the HTML default and the browser would be handed the whole page where it expected a stream. `handle` therefore branches on the events view *before* the rendering fork; `/events`, `/api/events` and `/events.json` all resolve to the same stream, because there is no other honest thing for them to be. The response deliberately carries **no `content-length`** (a stream that never ends has no length, and declaring one makes a browser buffer forever) and the same `nosniff` / `no-referrer` / CSP hardening every other response carries.
- **The read-only guarantee is untouched, and got it for free.** §24.1.1 decided the method check runs **before routing**, so the new route was already 405-with-`Allow: GET, HEAD` for every write method the moment it existed — which is the design working as intended rather than a new defence. The 405 matrix covers it explicitly anyway, in the suite and in `live-verify:viewer` (72 method×route combinations). SSE is a GET stream; no write route appears.
- **The CSP needed NO change, and that is worth stating rather than assuming.** EventSource is governed by **`connect-src`**, and the header shipped in wave 1 already reads `connect-src 'self'` under `default-src 'none'` — so a same-origin stream was permitted from the start and an off-origin one is still impossible. **CSP delta: zero bytes.** A test now pins `connect-src 'self'`, because the failure mode if a future minimization drops it is the nastiest kind: a page that looks completely fine and silently never updates.
- **Ephemerality is a contract, and the stream is the thing most likely to break it.** An SSE response is a socket the server holds open indefinitely — precisely what would make `server.close()` wait forever on a response only this process will ever finish, producing the resident-daemon behaviour §24.1 exists to avoid. So: both timers are **`unref`'d**, they exist **only while a client is attached** (no client, no work at all — and no timer to forget), and `close()` **ends every open stream first**, before `closeAllConnections`, so the client sees a clean end rather than a reset. A request that races the shutdown gets an immediately-ended stream rather than a reference parked in a set nobody will drain again.
- **The drop UX is the honest one, and it is forced by the lifecycle.** This server is ephemeral: Ctrl-C ends it and the next `cmk view` binds a **different** port — so when the stream dies there is **nothing on this page's origin to reconnect to, ever**. A browser's EventSource does not know that and retries forever, which would leave a dead tab quietly burning reconnects. So the page counts failures, gives up after **3** (`LIVE_GIVE_UP`), closes the stream itself, and says so in the freshness stamp: *"live off (server stopped — restart `cmk view` and reload)"*. **The two things this must never be are the two easy ones** — silent staleness (a page that looks live and is a photograph, the worst available failure on a memory viewer) and infinite retry noise. The freshness stamp is the surface for all three states because it is already the page's one honest statement about age; it now composes a base half (what this view read, and when) with a live half (whether the page is still being told about changes), since either can repaint without the other.
- **The reader's position and INPUT never move — which took two exceptions, not one (Task 259 review, I2).** The graph was the anticipated one. The other is the **search box**: `render()` repaints `#q` from the URL, so a `change` landing between a keystroke and its 220 ms debounce would discard what the reader was mid-way through typing — the loudest possible way to move someone's position, on the feature whose headline claim is that nothing moves. Both resolve identically (badge, don't repaint), and the typing test is deliberately **two** conditions because either alone has a real miss: `document.activeElement === #q` covers "typed three characters and paused to think" (no timer pending, but the box is theirs), and a pending debounce covers "typed and blurred inside 220 ms" (not focused, but a search is already on its way). The debounce handle doubles as the second flag, so it is **nulled when it fires** — a spent id left in place reads as permanently-typing and live refresh would never repaint again.
- **The stream's lifecycle is the tab's, and both directions are handled (I4 + I5).** **Back/forward cache:** a `pagehide`-only teardown is a one-way door — the browser restores the page with its JS state intact, including a stamp still reading `· live` over a stream closed on the way out, which is the silent-staleness failure arriving from the one direction nobody tests. `pageshow` with `persisted` corrects the stamp **first** and then re-opens, so a failed re-open cannot leave the false claim standing. **Socket budget:** HTTP/1.1 allows ~6 connections per origin and every open tab holds its SSE response for its whole life; multi-tab is a supported interaction here (ctrl-click passes straight through to the browser by design), so a hidden tab **releases** its stream and a visible one takes it back. Re-opening also **re-reads**, because a stream cannot report changes that happened while nobody was listening; it never resurrects over a `gone` verdict the reader has already been shown; and `startLive` is idempotent, or visibility flapping would open N streams. **Residual, accepted and stated in the source:** six or more *concurrently visible* windows on this origin can still saturate the budget — not mitigated, because reaching it needs several side-by-side windows of one ephemeral loopback viewer and the honest fixes (a shared worker, a token) are real machinery against a case that does not occur.
- **A successful Refresh retires `gone` (M1).** `gone` is an *inference* from three failed reconnects, not an observation; a fetch that **succeeds** against the same origin is direct evidence the inference has expired, so the page clears the verdict and tries the stream again. Leaving "server stopped" on screen while the page demonstrably just served a request is the same lying-control class as a stale badge under a refreshed view. Only a refresh that actually **resolved** may clear it — a failed one leaves the verdict standing.
- **Every view auto-refreshes in place EXCEPT the graph.** A `change` re-runs the current view through the same API with the same URL state, so nothing about the reader's position moves. The graph is the exception and it is deliberate: it re-runs a force simulation, so an automatic refresh would move **every** node — possibly the one under the reader's cursor, mid-click. It gets told instead (*"memory changed — Refresh to see them"*) and redraws when asked. This is why the manual Refresh control survives the automatic one: it is the fallback when the stream is gone, and the only control on the graph.
- **Zero new dependencies** (§24.1 point 7, unchanged): node's own `http` writing the wire format directly, the browser's built-in `EventSource`, no websocket library. **No new CLI flag** — the poll cadence is a server-internal constant, not a `?poll=` parameter, because a client-settable one would let a bookmarked URL turn a read-only viewer into a busy loop. `startViewer({pollMs})` is a test seam only.
- **What automated coverage cannot reach, stated rather than implied.** `live-verify:viewer` speaks SSE itself (Node has no `EventSource`, and adding one would break the zero-dependency rule for a gate script) — which proves the *bytes* are a well-formed event stream and closes the real loop end-to-end: a real `cmk remember` in one process, the real 2 s poll in another, a `change` frame on a held-open socket. **It closes the queue loop too** (the I1 half): an append to `queues/review.md` on the same open stream must produce a second `change` — written directly rather than through a bin, on the fingerprint entry's own terms, because that entry exists as the belt for writers that are *not* the kit, and because an append to an existing file is the harder case (it does not move the containing directory's mtime, which is why the two queue files are named individually). The audit half of I1 is a unit test's job — reaching `routeMedium` from a bin needs a live medium-trust extraction. Reverting the fingerprint entry reproduces the reviewer's probe exactly: 30 s of polling, no event. It does **not** prove the CLIENT half: a browser EventSource actually re-rendering on `change`, the give-up indicator, and the graph's refresh-when-asked badge each need a human with a tab open (the same standing caveat §24.1.2 states for the rendered page).
...
- **No fs.watch anywhere, in wave 1 OR wave 2** — the one platform-sensitive moving part, deliberately excluded. **Original wave-1 framing (2026-08-02):** "excluded so the first ship has zero such parts; Task 259 owns it — when it builds, evaluate polling-mtime as the watch mechanism before fs.watch." **Outcome (Task 259, 2026-08-09):** polling won and `fs.watch`/chokidar was never written, so the exclusion is now permanent rather than deferred — see §24.1.3 for the evidence (D-329's 5/5 dropped events under load) and the mechanism that replaced it.
