...
- **Two budgets** (both registered in `validate-budget-pairs`): `VIEWER_DEFAULT_LIMIT=50` / `VIEWER_MAX_LIMIT=200` for the facts list — an over-cap `?limit=` **clamps** and reports `clamped:true` rather than 400ing a URL someone typed; and the graph's `node_limit`, which budgets **live facts only** — anchor hubs and archived-superseded predecessors ride on top and are counted separately, because a supersession arrow with no tail is not a supersession arrow. Revisit trigger: a corpus where `archived_count + anchor_count` approaches `node_limit`.
