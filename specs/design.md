...
**The derivation.** Escape the alphabet's only lowercase letter: `a` → `_a`. `_` is outside the id alphabet, so under case folding the per-character images are all distinct AND prefix-free, which makes the map injective — two distinct ids can never produce one case-folded name. It is a **no-op for the ~78% of ids containing no `a`**, so the overwhelming majority of archive files keep the name they already had.
