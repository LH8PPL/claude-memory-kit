...
### 24.1.2 The visual contract (Task 260, 2026-08-03 — the visual pass, D-420; Task 268, 2026-08-06 — the redesign, D-425; direction revised 2026-08-08 to dark-default, D-432)
...
- **One token block per theme, and both are DESIGNED — neither is an inversion of the other.** A warm neutral ladder — `--ground` / `--panel` / `--sunken` / `--line` / `--line-soft` and three inks — redefined in full for the other theme rather than inverted. The page and the panel must differ by a real step: the shipped v1 was `#fbfaf8` on `#ffffff`, a 2% delta, which left 1px borders doing 100% of the separation. **Since D-432 the DEFAULT block (`:root`) is dark** and light is served under `prefers-color-scheme: light`. Each theme is served on **two signals** — the system preference and an explicit `:root[data-theme="light"|"dark"]` — so a toggle wins in both directions; the two bodies of each palette are byte-identical duplicates on purpose (a derived form needs an indirection layer through every hue, and the contrast test can only compute what it can parse), and a test pins them to each other so a `data-theme` copy cannot drift out of AA coverage. Tier hue-coding is a **light-only** encoding: with one accent that doubles as `--warn`, dark has no hue budget to spend three ways, so its tier glyphs are monochrome and the LETTER carries the meaning.
