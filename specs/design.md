...
- **The fact card has THREE typographic tiers** — title (15/600) · snippet (13.5, second ink) · meta (12, third ink) — over a `--measure` reading column (760px). The server still sends ONE flattened string per fact: **the title/body split is a client concern** (first sentence, then dash/colon, then a clause break, then a word break), and the overflow is a CSS `-webkit-line-clamp`, not more server-side `…` — the whole string stays in the DOM and the accessibility tree.
