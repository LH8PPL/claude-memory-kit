---
id: P-C3PDGYHM
aliases: [P-C3PDGYHM]
type: project
shape: Timeless
title: Healthy Corpus Baseline Metrics
created_at: 2026-08-10T11:36:14Z
write_source: auto-extract
trust: medium
recurrence_count: 1
source_file: auto-extract
source_line: 1
source_sha1: 89774e8a56f9f8c922f48b7af411c2a0853c6201de7c3d0f02895ad58da50e06
related: [v0-6-2-published-release-state-and-contents, corpus-poison-repair-command]
---

A known-healthy corpus shows:
- Floor (minimum linked count): ≥ 0.1585
- Example healthy run: 246 linked items observed

**Why:** Distinguishes normal operation from degraded/poisoned corpus state

**How to apply:** Use as baseline for corpus-health assertions in tests and probes
