---
id: P-BH25FCLN
type: project
shape: Absence
title: youtube-to-slide foreign-format fact file — pre-existing, not rename-related
created_at: 2026-07-15T06:59:28Z
write_source: auto-extract
trust: medium
recurrence_count: 1
source_file: auto-extract
source_line: 1
source_sha1: 9efa4976015c02521b0564c5e7f413e28d6e450a6b11f8b739ace1dd593f423f
related: [fact-file-write-constraint-forward-only-no-mass-rewrites, hc-2-distill-freshness-fail-self-clears, hc-9-drift-after-claude-code-update-v0-3-4]
---

One fact file in youtube-to-slide fails HC-4 validation; user data, not a rename bug

**Why:** Clarify that v0.5.4 rename is clean; this is pre-existing data quality, not caused by rebrand

**How to apply:** If youtube-to-slide HC-4 shows a validation fail, it's the known foreign-format file (not a regression)
